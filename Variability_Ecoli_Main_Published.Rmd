...
Section 2: Load the Mut dataset (this study) and calculate transcriptional variability (DMmut)
Section 3: Load the Env dataset (iModulonDB, PRECISE2, E.coli) and calculate transcriptional variability (DMenv) 
Section 4: Load the Evo dataset (Rousset, Nat. Microb., 2021) and calculate transcriptional variability (DMevo)
...
Section 11: Analysis of the Rwr dataset (Baumstark, Nat. Comm., 2015)
Section 12: Analysis of the Random datasets (Computationally generated)
Section 13: Analysis of the Noise_ch dataset (Taniguchi, Science, 2010)
Section 14: Analysis of the Prtn dataset (Schmidt, Nat. Biotechnol., 2016)
Section 15: Analysis of the Strs dataset (Maeda and Iwasawa, Nat. Commun., 2020)
Section 16: Analysis of the Strs_hp and Strs_he (Horinouchi, Sci. Rep., 2017)
Section 17: Bow-tie decomposition
Section 18: Summary
Section 19: Output for supplementary tables
...
mydscolpal<-c("#00AFBB", "#E7B800", "#FC4E07")
mygocolpal<-c("#42B540FF","#925E9FFF","#DF8F44FF")
Sys.time()
```
...
```{r Prep_Namespace,warning=FALSE, message=FALSE,fig.height=1.4,fig.width=1.4,dpi=300}
# gene name of W3110
...
```{r Prep_TRN,warning=FALSE, message=FALSE,fig.height=2,fig.width=2,dpi=300}
...
Sys.time()
```
## 1.3. Prepare KEGG pathways (latest)
...
Sys.time()
```
## 1.4. Prepare KEGG pathways (previous dataset, default)
```{r Prep_gseKEGG,warning=FALSE, message=FALSE,fig.height=4,fig.width=4,dpi=300}
...
for(i in 1:nrow(dkegg_prev)){
  ...
  dkegg_prev$EntrezID[i]<-dBnb.comp[dBnb.comp$Bnb %in% my_tmp,]$EntrezID %>% paste(.,collapse = ",")
}
...
my_df<-dBnb.comp%>% select(EntrezID,Bnb)
...
ggscatter(dat,x="AncHa_rep1_0",y="H03_30",alpha=0.1, size = 0.2,
          ...
          xlab="mRNA expression level of Anc [arb. unit]", 
          ylab="mRNA expression level of Evo [arb. unit]")+
...
The raw linear data was log2-transformed.
...
ggscatter(datlog.norm,x="AncHa_rep1_0",y="H03_30",alpha=0.1,size = 0.2)+
  xlab(expression(log[2]~"mRNA expression level of Anc [arb. unit]"))+
  ylab(expression(log[2]~"mRNA expression level of Evo [arb. unit]"))+
...
ggscatter(datlog.norm,x="AncHa_rep1_0",y="AncHa_rep2_0",
  ...
  xlab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 1 [arb. unit]"))))+
  ylab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 2 [arb. unit]"))))
#TODO ggsave(file="FigS1c.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)
...
ggscatter(datlog.norm,x="AncHa.mean",y="H03_30",
          ...
          legend.title="")+
  geom_abline(slope = 1,intercept = 0,color=mycolpal[2])+
  xlab(expression(atop(log[2]~"mRNA expression level", paste("of AncHa [arb. unit]"))))+
  ylab(expression(atop(log[2]~"mRNA expression level", paste("of AncHa_30 [arb. unit]"))))
...
ggscatter(datlog.norm,x="AncHb_rep1_0",y="AncHb_rep2_0",
  ...
  xlab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 1 [arb. unit]"))))+
  ylab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 2 [arb. unit]"))))
#TODO ggsave(file="FigS1d.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)
...
ggscatter(datlog.norm,x="AncHb.mean",y="H08_30",
          ...
          legend.title="")+
  geom_abline(slope = 1,intercept = 0,color=mycolpal[2])+
  xlab(expression(atop(log[2]~"mRNA expression level", paste("of AncHb [arb. unit]"))))+
  ylab(expression(atop(log[2]~"mRNA expression level", paste("of AncHb_30 [arb. unit]"))))
...
```{r Mut,warning=FALSE, message=FALSE,fig.height=3.5,fig.width=4.5,dpi=300}
my_fontsize<-16
p<-ggscatter(datlog.norm,x="MEANmut",y="SDmut", alpha = 0.3, size=1.5,fill="black",
              ...
              xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
              ylab(expression("Standard deviation [arb. unit]"))+
...
p
#TODO ggsave(file="Fig1e_Mut.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5,bg=NULL)
...
p<-ggscatter(datlog.norm,x="MEANmut",y="DMmut", alpha = 0.3, size=1.5,fill="black",
    ...
    scale_x_continuous(expand = expansion(mult = c(0.1, 0.2)),breaks=get_breaks(from = 4,to = 16,by = 4))+
          xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
          ylab(expression(DM[mut]~"[arb. unit]"))
p<-ggpar(p,font.x=my_fontsize,font.y=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig1f_Mut.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5)
...
p<-ggerrorplot(temp2,x="gene",y="log_ex_norm",desc_stat = "mean_sd",
            ...
            xlab = "")+
  # ylab(expression(log[2]~"mRNA expression level [arb. unit]"))+
  ylab(expression(atop(log[2]~"mRNA expression", paste("level [arb. unit]"))))+
...
p
#TODO ggsave(file="Fig1d_Mut.svg", path=my_figout,dpi = 300, width = 4, height = 3)
Sys.time()
```
...
my_df<-datlog.norm %>% select(Bnb,MAlin.sel)
my_mat<-my_df %>% select(-Bnb)
my_df1<-data.frame(sample=MAlin.sel,
                   Group=c(rep(c(rep("A",3),rep("B",3)),2),"A","B"),
                   Round=c(c(rep(10,6),rep(30,6)),0,0))

ha_column = HeatmapAnnotation(df = data.frame(Group = my_df1$Group,
                                              Round = my_df1$Round), 
                              col = list(Group = c("A" = mycolpal[1], "B" = mycolpal[2]),
                                         Round = c("0" = "gray80", "10" = "gray50","30"="gray20")))
...
Heatmap(my_mat,
        ...
        heatmap_legend_param = list(title="ELV"))
...
Envdat_cor<-fileinfo_plasticity.pr2
Envdat_cor$cor<-NA
my_col<-Envdat_cor$sample_group %>% unique()
i<-1
for(i in 1:length(my_col)){
  my_df<-log_tpm_pl.pr2 %>% select(Envdat_cor[Envdat_cor$sample_group==my_col[i],]$sample)
  my_res<-my_df %>% cor_test(method = "spearman")
  Envdat_cor[Envdat_cor$sample_group==my_col[i],]$cor<-my_res[my_res$var1!=my_res$var2,]$cor %>% mean()
}

Envdat_cor<-Envdat_cor %>% distinct(sample_group,.keep_all = T)

gghistogram(Envdat_cor,x="cor",fill = "black",xlim=c(0.95,1),size=0,bins = 6)

ggplot(Envdat_cor, aes(x = cor)) +
  geom_histogram(
    breaks = seq(0.95, 1, by = 0.01))+
  theme_pubr()+
  xlab("Spearman's R")

# representative scatter
ggscatter(log_tpm_pl.pr2,x="control__wt_glc__1",y="control__wt_glc__2",
          alpha=0.1,size=0.2,
          cor.coef = T,cor.method = "spearman",
          legend.title="")+
  geom_abline(slope = 1,intercept = 0,color=mycolpal[2])+
  xlab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 1 [arb. unit]"))))+
  ylab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 2 [arb. unit]"))))
#TODO ggsave(file="FigS1b.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

# representative scatter
ggscatter(log_tpm_pl.pr2,x="rpoB__wt_lb__1",y="rpoB__wt_lb__2",
          alpha=0.1,size=0.2,
          cor.coef = T,cor.method = "spearman",
          legend.title="")+
  geom_abline(slope = 1,intercept = 0,color=mycolpal[2])+
  xlab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 1 [arb. unit]"))))+
  ylab(expression(atop(log[2]~"mRNA expression level", paste("of replicate 2 [arb. unit]"))))
#TODO ggsave(file="FigS1a.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

nrow(Envdat_cor[Envdat_cor$cor>0.98,])/nrow(Envdat_cor)

Sys.time()
```
### 3.2.3. w/o the abx project
```{r Env,warning=FALSE, message=FALSE,fig.height=1.5,fig.width=1.5,dpi=300}
...
```{r Env,warning=FALSE, message=FALSE,fig.height=3.5,fig.width=4.5,dpi=300}
my_fontsize<-16
# Mean vs SD in PR2
...
p<-ggpar(p,font.x=my_fontsize,font.y=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig1e_Env.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5)
...
p<-ggscatter(prtemp2.pr2,x="MEANenv",y="DMenv", alpha = 0.3,size=1.5,
  ...
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.3)))+
          xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
          ylab(expression(DM[env]~"[arb. unit]"))
p<-ggpar(p,font.x=my_fontsize,font.y=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig1f_Env.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5)
...
p<-ggpar(p, xlab = FALSE, font.tickslab = list(size=my_fontsize),font.y = list(size=my_fontsize))
p
#TODO ggsave(file="Fig1d_Env.svg", path=my_figout,dpi = 300, width = 4, height = 3)
Sys.time()
```
...
```
## 3.7. Growth rate
```{r Env,warning=FALSE, message=FALSE,fig.height=6,fig.width=6,dpi=300}
my_df<-fileinfo_plasticity.pr2[!is.na(fileinfo_plasticity.pr2$Growth.Rate..1.hr.),]
my_df1<-my_df[my_df$rep_id==1,]
for (i in 1:nrow(my_df1)){
  my_tmp<-my_df[my_df$sample_group==my_df1$sample_group[i],]
  my_df1$Growth.Rate..1.hr.[i]<-my_tmp$Growth.Rate..1.hr.%>% mean()
}
my_df1$Carbon<-NA
my_df1$Nitrogen<-NA
my_df1$Aerobic<-"Aerobic"
my_df1[my_df1$Electron.Acceptor!="O2",]$Aerobic<-"Anaerobic"
my_df1$Supplements<-NA
i<-1
for(i in 1:nrow(my_df1)){
  my_tmp<-str_split(my_df1$Carbon.Source..g.L.[i],pattern = "\\(")  %>% unlist()
  my_df1$Carbon[i]<-my_tmp[1]
  my_tmp<-str_split(my_df1$Nitrogen.Source..g.L.[i],pattern = "\\(")  %>% unlist()
  my_df1$Nitrogen[i]<-my_tmp[1]
}

my_df1<-my_df1 %>% select(sample_group,Growth.Rate..1.hr.,Base.Media,Carbon,Nitrogen,Aerobic,Supplement,Trace.Element.Mixture,pH,Temperature..C.,condition)
my_df1<-my_df1[order(my_df1$Growth.Rate..1.hr.,decreasing = T),]
my_df1$order<-seq(1,nrow(my_df1))
my_df1$y<-1

# Base media
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=Base.Media),color="grey30")+scale_fill_manual(values =c("black","grey80"))+ # base media
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
  theme_pubr(legend = "right")
ggpar(p,legend.title = "",xlab="",ylab="",tickslab = F,ticks = F)


# Carbon source
my_uniq<-my_df1$Carbon %>% unique()
my_level<-c("","glucose",my_uniq[my_uniq %nin% c("","glucose")])
my_df1$Carbon<-factor(my_df1$Carbon,levels = my_level)
my_col_level<-c("white",rev(get_palette(palette = "Reds", k = c(length(my_level)-1))))
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=Carbon),color="grey30")+scale_fill_manual(values =my_col_level)+ # carbon source
  # geom_tile(aes(fill=Supplement),color="grey30")+# supplement
  # geom_tile(aes(fill=pH),color="grey30")+# supplement
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
  theme_pubr(legend = "right")
ggpar(p,legend.title = "",xlab="",ylab="",tickslab = F,ticks = F)
...
# Supplement
my_uniq<-my_df1$Supplement %>% unique()
my_level<-c(my_uniq[my_uniq %nin% c("salicylate","DPD (0.2mM)","PQ (250uM)","NaCl (0.3M)")],c("salicylate","DPD (0.2mM)","PQ (250uM)","NaCl (0.3M)"))
my_df1$Supplement<-factor(my_df1$Supplement,levels = my_level)
my_col_level<-c("white",rev(get_palette(palette = "RdPu", k = c(length(my_level)-5))),get_palette(palette = "Blues", k = 4))
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=Supplement),color="grey30")+scale_fill_manual(values =my_col_level)+ # carbon source
  # geom_tile(aes(fill=Supplement),color="grey30")+# supplement
  # geom_tile(aes(fill=pH),color="grey30")+# supplement
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
...
# Aerobic
my_uniq<-my_df1$Aerobic %>% unique()
my_level<-my_uniq
my_df1$Aerobic<-factor(my_df1$Aerobic,levels = my_level)
my_col_level<-c("#00AFBB", "#E7B800")
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=Aerobic),color="grey30")+scale_fill_manual(values =my_col_level)+ # carbon source
  # geom_tile(aes(fill=Supplement),color="grey30")+# supplement
  # geom_tile(aes(fill=pH),color="grey30")+# supplement
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
...
# Temperature
my_uniq<-my_df1$Temperature..C. %>% unique()
my_level<-my_uniq
my_df1$Temperature..C.<-factor(my_df1$Temperature..C.,levels = my_level)
my_col_level<-c("#F9A825", "#F57F17")
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=Temperature..C.),color="grey30")+scale_fill_manual(values =my_col_level)+ # carbon source
  # geom_tile(aes(fill=Supplement),color="grey30")+# supplement
  # geom_tile(aes(fill=pH),color="grey30")+# supplement
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
...
# Trace elements
my_uniq<-my_df1$Trace.Element.Mixture %>% unique()
my_level<-my_uniq
my_df1$Trace.Element.Mixture<-factor(my_df1$Trace.Element.Mixture,levels = my_level)
my_col_level<-c("white","#00C853","#64DD17", "#B9F6CA")
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=Trace.Element.Mixture),color="grey30")+scale_fill_manual(values =my_col_level)+ # carbon source
  # geom_tile(aes(fill=Supplement),color="grey30")+# supplement
  # geom_tile(aes(fill=pH),color="grey30")+# supplement
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
...
# pH
my_uniq<-my_df1$pH %>% unique()
my_level<-c(5.5,7.0,8.5)
my_df1$pH<-factor(my_df1$pH,levels = my_level)
my_col_level<-c("#C0CA33","#00897B","#039BE5")
p<-ggplot(my_df1,aes(order,y))+
  geom_tile(aes(fill=pH),color="grey30")+scale_fill_manual(values =my_col_level)+ # carbon source
  # geom_tile(aes(fill=Supplement),color="grey30")+# supplement
  # geom_tile(aes(fill=pH),color="grey30")+# supplement
  theme(panel.grid = element_blank(),
        axis.text = element_blank()) +
...
ggbarplot(my_df1,y="Growth.Rate..1.hr.",x="sample_group",xlab="",fill="grey50",size=0)+
  rotate_x_text()+
...
Sys.time()
```
## 3.8. The Envexp dataset (lmitied to samples obtained at exponential growth phase)
```{r Envexp,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
df_growth_phase<-paste(getwd(),"ENV_DataSet","data_files","growth_phase.xlsx",sep="/") %>% read_xlsx()
df_growth_phase<-df_growth_phase %>% select(sample_ID,GrowthPhase)
df_growth_phase<-left_join(fileinfo_plasticity.pr2,df_growth_phase)

log_tpm_grlog<-log_tpm.pr2.norm
log_tpm_grlog<-log_tpm_grlog %>% select(Bnb,df_growth_phase[df_growth_phase$GrowthPhase=="log",]$sample_ID)
df_env_grlog<-data.frame(Bnb=log_tpm_grlog$Bnb,MEANenvexp=NA,SDenvexp=NA,RMenvexp=NA,SSenvexp=NA,DMenvexp=NA)
df_env_grlog$MEANenvexp<-log_tpm_grlog %>% select(-Bnb) %>% as.matrix() %>% rowMeans(na.rm=T)
df_env_grlog$SDenvexp<-log_tpm_grlog %>% select(-Bnb) %>% as.matrix() %>% rowSds(na.rm=T)
df_env_grlog<-df_env_grlog[order(df_env_grlog$MEANenvexp),]
df_env_grlog$RMenvexp<-runmed(df_env_grlog$SDenvexp,41)
my_list<-list()
my_list[[1]]<-smooth.spline(data.frame(x=df_env_grlog$MEANenvexp,y=df_env_grlog$RMenvexp),df=12)
my_tmp<-createEmptyDf(nrow=1000,ncol=2, colnames = qw(logx,SS))
my_tmp$logx<-seq(min(df_env_grlog$MEANenvexp),max(df_env_grlog$MEANenvexp),length=nrow(my_tmp))
my_tmp$SS<-predict(my_list[[1]],my_tmp$logx)[[2]]
df_env_grlog$SSenvexp<-predict(my_list[[1]],df_env_grlog$MEANenvexp)[[2]]
df_env_grlog$DMenvexp<-df_env_grlog$SDenvexp-df_env_grlog$SSenvexp

my_df1<-prtemp2.pr2 %>% select(Bnb,MEANenv,DMenv)
my_df<-left_join(df_env_grlog,my_df1)

ggscatter(my_df,x="DMenv",y="DMenvexp",
          alpha=0.1,size=0.2,
          cor.coef = T,cor.method = "spearman",
          legend.title="")+
  geom_abline(slope = 1,intercept = 0,color=mycolpal[2])+
  xlab(expression(DM[env]~"[arb. unit]"))+
  ylab(expression(DM[envexp]~"[arb. unit]"))

```
## 3.9. Sample information about the Envexp dataset
```{r Envexo,warning=FALSE, message=FALSE,fig.height=4,fig.width=4,dpi=300}
df_growth_phase[df_growth_phase$GrowthPhase=="log",]$condition_concat %>% unique()
nrow(df_growth_phase[df_growth_phase$GrowthPhase=="log",])
# Sys.time()
```
# 4. The Evo dataset
...
#   labs(x="DMenv [a.u]",y="DMevo [arb. unit]")
# 
...
#   labs(x="DMmut [a.u]",y="DMevo [arb. unit]")
...
```{r Evo,warning=FALSE, message=FALSE,fig.height=3.5,fig.width=4.5,dpi=300}
my_fontsize<-16
...
p<-ggpar(p,font.x=my_fontsize,font.y=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig1e_Evo.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5)
...
p<-ggscatter(dcol16.qnorm,x="MEANevo",y="DMevo", alpha = 0.3, size=1.5,fill="black",
  ...
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.3)))+
          ...
          ylab(expression(DM[evo]~"[arb. unit]"))
p<-ggpar(p,font.x=my_fontsize,font.y=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig1f_Evo.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5)
...
p<-ggpar(p, xlab = FALSE, font.tickslab = list(size=my_fontsize),font.y = list(size=my_fontsize))
p
#TODO ggsave(file="Fig1d_Evo.svg", path=my_figout,dpi = 300, width = 4, height = 3)
Sys.time()
```
...
p
#TODO ggsave(file="Fig1b.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
Sys.time()
```
## 4.7. Representative two genes in Eco16
...
```
## 4.8. Heatmap
```{r Evo,warning=FALSE, message=FALSE,fig.height=4,fig.width=4,dpi=300}
my_df<-dcol16.qnorm %>% select(Bnb,my_strains$Condition)
my_mat<-my_df %>% select(-Bnb)
my_df1<-data.frame(sample=my_strains$Condition,
                   Phylogroup=my_strains$Phylogroup)

ha_column = HeatmapAnnotation(df = data.frame(Phylogroup=my_df1$Phylogroup), 
                              col = list(Phylogroup = c("A" = gg_color_hue(6)[1], 
                                                        "B1" = gg_color_hue(6)[2],
                                                        "B2"=gg_color_hue(6)[3],
                                                        "D"=gg_color_hue(6)[4],
                                                        "E"=gg_color_hue(6)[5],
...
p
#TODO ggsave(file="Fig2a_EnvMut.svg", path=my_figout,dpi = 300, width = 2.5, height = 2.5)
...
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig2a_EnvEvo.svg", path=my_figout,dpi = 300, width = 2.5, height = 2.5)
...
p<-ggscatter(df_DMall,x="DMmut",y="DMevo",
  ...
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.3)))+
          xlab(expression(DM[mut]~"[arb. unit]"))+
          ylab(expression(DM[evo]~"[arb. unit]"))
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="Fig2a_MutEvo.svg", path=my_figout,dpi = 300, width = 2.5, height = 2.5)
...
mypathway<-qw(eco00260,eco00790,eco00970,eco04122,eco02030)
df_DMall_cat[df_DMall_cat$Pathway_ID %in% mypathway,]$label.sel<-df_DMall_cat[df_DMall_cat$Pathway_ID %in% mypathway,]$label %>% as.character()
...
p<-ggscatter(df_DMall_cat,x="DMenv",y="DMmut",
          ...
          xlab(expression("Mean"~DM[env]~"[arb. unit]"))+
          ylab(expression("Mean"~DM[mut]~"[arb. unit]"))+
...
p
#TODO ggsave(file="FigS5_EnvMut.svg",path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)
...
p<-ggscatter(df_DMall_cat,x="DMenv",y="DMevo",
          ...
          cor.coeff.args = list(method = "spearman",label.sep = "\n",size=4,label.x=0,label.y=1))+
          xlab(expression("Mean"~DM[env]~"[arb. unit]"))+
          ylab(expression("Mean"~DM[evo]~"[arb. unit]"))+
  geom_text_repel(data=df_DMall_cat,
                  aes(label=label.sel),segment.size=0.3,segment.alpha = 0.5,nudge_x = 0.6, lineheight = 0.1,size=3)
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.xtickslab = my_fontsize,font.ytickslab = my_fontsize)
p
#TODO ggsave(file="FigS5_EnvEvo.svg",path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)
...
p<-ggscatter(df_DMall_cat,x="DMmut",y="DMevo",
          ...
          xlab(expression("Mean"~DM[mut]~"[arb. unit]"))+
          ylab(expression("Mean"~DM[evo]~"[arb. unit]"))+
  geom_text_repel(data=df_DMall_cat,
                  aes(label=label.sel),segment.size=0.3,segment.alpha = 0.5,nudge_x = 0.6, lineheight = 0.1,size=3)
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.xtickslab = my_fontsize,font.ytickslab = my_fontsize)
p
#TODO ggsave(file="FigS5_MutEvo.svg",path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)
...
#TODO ggsave(file="FigS5_Table.svg",path=my_supplfigout,dpi = 300, width = 5, height = 2,bg=NULL)
Sys.time()
```
## 5.4.The CellCycle dataset
https://www.nature.com/articles/s41586-023-06974-w#Sec30
Supplementary Table 5 & 6
### 5.4.1.
```{r All_DM,warning=FALSE, message=FALSE,fig.height=4,fig.width=16,dpi=300}
df_trip_each<-paste(getwd(),"CellCycle_DataSet","41586_2023_6974_MOESM4_ESM.xlsx",sep = "/") %>% read_xlsx(sheet = "eco_lb_1")
df_trip_each<-df_trip_each %>% pivot_longer(cols = -cell_angle,names_to = "Bnb",values_to = "MAE") # MAE: mean-adjusted expression
df_trip_each<-df_trip_each[df_trip_each$Bnb %in% dBnb.comp$Bnb,]
df_trip_each<-df_trip_each
ggline(df_trip_each,x="cell_angle",y="MAE",color="Bnb",numeric.x.axis = T,legend="",plot_type = "l",alpha=0.1,size=0.1)

df_trip<-paste(getwd(),"CellCycle_DataSet","41586_2023_6974_MOESM3_ESM.xlsx",sep = "/") %>% read_xlsx(sheet = "eco_lb_1")
colnames(df_trip)[1]<-"Bnb"
df_trip<-df_trip[df_trip$Bnb %in% dBnb.comp$Bnb,]
df_trip$cluster<-df_trip$cluster %>% as.character()
df_trip[is.na(df_trip$cluster),]$cluster<-"others"
df_trip$cluster<-factor(df_trip$cluster,levels=c(seq(0,19),"others"))
my_tmp<-df_DMall %>% select(Bnb,DMenv,DMevo,DMmut)
df_trip<-left_join(df_trip,my_tmp)

df_trip$origin_distance<-df_trip$origin_distance/1000


trip_stat<-data.frame(cluster=c(seq(0,19),"others"),count=0)
for(i in 1:nrow(trip_stat)){
  trip_stat$count[i]<-df_trip[df_trip$cluster==trip_stat$cluster[i],] %>% nrow()
}

figlist<-list()
for(i in 0:19){
  p<-ggline(df_trip_each[df_trip_each$Bnb %in% df_trip[df_trip$cluster==i,]$Bnb,],x="cell_angle",y="MAE",color="Bnb",numeric.x.axis = T,legend="",plot_type = "l",alpha=0.1,size=0.1,ylim=c(0,3))+annotate("text",x=10,y=2.8,label=paste("cluster:",i,sep=" "),hjust = 0,vjust = 0, parse=F,size=6)
  figlist[[i+1]]<-p
}
k<-0
for(j in 1:5){
  grid.arrange(figlist[[k+1]],figlist[[k+2]],figlist[[k+3]],figlist[[k+4]],nrow=1)
  k<-k+4
}
...
```{r All_DM,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# Fold Change vs. DMs
ggscatter(df_trip,x="fold_change",y="DMenv",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 5, label.y=2, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Fold change (Pountain et al. 2024)")+
  ylab(expression(DM[env]~"[arb. unit]"))
#TODO ggsave(file="FigS2a_Env.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

ggscatter(df_trip,x="fold_change",y="DMevo",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 4.7, label.y=1, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Fold change (Pountain et al. 2024)")+
  ylab(expression(DM[evo]~"[arb. unit]"))
#TODO ggsave(file="FigS2a_Evo.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

ggscatter(df_trip,x="fold_change",y="DMmut",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 4.7, label.y=2.5, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Fold change (Pountain et al. 2024)")+
  ylab(expression(DM[mut]~"[arb. unit]"))
#TODO ggsave(file="FigS2a_Mut.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

# Distance from OriC vs. DMs
ggscatter(df_trip,x="origin_distance",y="DMenv",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 800, label.y=2.1, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Distance from oriC [1000 bp]")+
  ylab(expression(DM[env]~"[arb. unit]"))
#TODO ggsave(file="FigS2b_Env.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

ggscatter(df_trip,x="origin_distance",y="DMevo",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 700, label.y=2, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Distance from oriC [1000 bp]")+
  ylab(expression(DM[evo]~"[arb. unit]"))
#TODO ggsave(file="FigS2b_Evo.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

ggscatter(df_trip,x="origin_distance",y="DMmut",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 1000, label.y=2.8, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Distance from oriC [1000 bp]")+
  ylab(expression(DM[mut]~"[arb. unit]"))
#TODO ggsave(file="FigS2b_Mut.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3,bg=NULL)

ggscatter(df_trip,x="origin_distance",y="theta_g",
          alpha=0.5,
          # color="cluster",legend="right",legend.title="Cluster",
          palette = c(gg_color_hue(20),"#1B1919FF"),
          cor.coef = T,
          cor.coeff.args = list(method="spearman", label.x = 5, label.y=2.5, label.sep = "\n",size=5),
          size=0.3)+
  xlab("Distance from OriC")+
  ylab(expression(theta[g]~""))


ggerrorplot(df_trip,x="cluster",y="DMenv",desc_stat = "mean_sd",
            add="jitter",add.params =list(size = 1, jitter = 0.3,alpha=0.4,color="black"),
            size=0.4,color=mycolpal[2],
            xlab = "Cluster")+
  stat_anova_test(label.x = 3,label.y = 2.3)+
  rotate_x_text()+
  ylab(expression(DM[env]~"[arb. unit]"))

ggerrorplot(df_trip,x="cluster",y="DMmut",desc_stat = "mean_sd",
            add="jitter",add.params =list(size = 1, jitter = 0.3,alpha=0.4,color="black"),
            size=0.4,color=mycolpal[2],
            xlab = "Cluster")+
  stat_anova_test(label.x = 3,label.y = 2.8)+
  rotate_x_text()+
  ylab(expression(DM[mut]~"[arb. unit]"))
Sys.time()
```
### 5.4.3. GSEA
```{r All_DM,warning=FALSE, message=FALSE,fig.height=8,fig.width=6,dpi=300}

gene_list <- df_trip %>% select(DMenv) %>% unlist() # Create a vector of the gene universe
names(gene_list)<-df_trip$Bnb
gene_list = sort(gene_list, decreasing = TRUE) # sort the list in decreasing order (required for clusterProfiler)
my_t2g<-df_trip %>% select(cluster,Bnb)
...
Sys.time()
```
### 5.4.5. Distance from oriC
```{r All_DM,warning=FALSE, message=FALSE,fig.height=3,fig.width=6,dpi=300}
genome_size<-4631469
oriC_site<-(3925744+3925975)/2
my_tmp<-genome_size-oriC_site
my_df<-dBnb.comp
my_df$Mid<-(my_df$Start+my_df$End)/2
my_df$Mid<-ifelse(my_df$Mid<oriC_site,my_df$Mid+my_tmp,my_df$Mid-oriC_site)
my_df$distance<-ifelse(my_df$Mid<genome_size/2,my_df$Mid,genome_size-my_df$Mid)
my_df$distance_k<-my_df$distance/1000
my_df<-my_df %>% select(Bnb,distance,distance_k,Mid,Strand)
my_tmp<-prtemp2.pr2 %>% select(Bnb,MEANenv,DMenv)
my_df<-left_join(my_df,my_tmp)
my_tmp<-datlog.norm %>% select(Bnb,MEANmut,DMmut)
my_df<-left_join(my_df,my_tmp)

p<-ggscatter(my_df,x="distance_k",y="MEANmut",
             alpha=0.4,size=1,
             cor.coef = TRUE,cor.coeff.args = list(method = "pearson", label.y = 3),
             # shape="Strand",
             xlab = "Distance from oriC [1000 bp]",
             legend="",legend.title="")+
  ylab(expression(atop(log[2]~"mRNA expression", paste("level [arb. unit]"))))+
...
my_df3<-Envdat_cor %>% select(sample_group)
my_df3$cor<-NA
my_df3$p<-NA

i<-1
for (i in 1:nrow(my_df3)){
  my_tmp<-fileinfo_plasticity.pr2[fileinfo_plasticity.pr2$sample_group==my_df3$sample_group[i],]$sample
  my_df1<-log_tpm_pl.pr2 %>% select(Bnb,my_tmp)
  my_df1$MeanExp<-my_df1 %>% select(my_tmp) %>% as.matrix() %>% rowMeans()
  my_df2<-left_join(my_df,my_df1)
  my_res<-my_df2 %>% select(distance_k,MeanExp) %>% cor_test(method = "spearman")
  my_df3$cor[i]<-my_res$cor
  my_df3$p[i]<-my_res$p
}

gghistogram(my_df3,x = "cor",y = "count",xlim=c(-0.2,0.2))

Sys.time()
```
## 5.5. Comparison between DMenv, DMevo, DMmut and DMenvexp
```{r All_DM,warning=FALSE, message=FALSE,fig.height=2.5,fig.width=2.5,dpi=300}
my_df<-df_DMall
my_tmp<-df_env_grlog %>% select(Bnb,DMenvexp)
my_df<-left_join(my_df,my_tmp)
...
# DMenvexp vs. DMenv
p<-ggscatter(my_df,x="DMenvexp",y="DMenv",
          size=0.2,alpha=0.2,
          color="gray20",
          cor.coef = T,
          cor.coeff.args = list(method = "spearman",label.sep = "\n",size=5,label.x=1,label.y=0.1))+
          xlab(expression(DM[envexp]~"[arb. unit]"))+
          ylab(expression(DM[env]~"[arb. unit]"))
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="FigS3_Envexp_Env.svg", path=my_supplfigout,dpi = 300, width = 2.5, height = 2.5,bg=NULL)


# DMenvexp vs. DMevo
p<-ggscatter(my_df,x="DMenvexp",y="DMevo",
          size=0.2,alpha=0.2,
          color="gray20",
          cor.coef = T,
          cor.coeff.args = list(method = "spearman",label.sep = "\n",size=5,label.x=-0.5,label.y=3.2))+
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.3)))+
          xlab(expression(DM[envexp]~"[arb. unit]"))+
          ylab(expression(DM[evo]~"[arb. unit]"))
...
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="FigS3_Envexp_Evo.svg", path=my_supplfigout,dpi = 300, width = 2.5, height = 2.5,bg=NULL)

# DMenvexp vs. DMmut
p<-ggscatter(my_df,x="DMenvexp",y="DMmut",
          size=0.2,alpha=0.2,
          color="gray20",
          cor.coef = T,
          cor.coeff.args = list(method = "spearman",label.sep = "\n",size=5,label.x=0.8,label.y=2.6))+
          xlab(expression(DM[envexp]~"[arb. unit]"))+
          ylab(expression(DM[mut]~"[arb. unit]"))
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.tickslab = my_fontsize)
p
#TODO ggsave(file="FigS3_Envexp_Mut.svg", path=my_supplfigout,dpi = 300, width = 2.5, height = 2.5,bg=NULL)
...
p<-ggdotchart(figtemp1,x="Path_lab",y="NES", # "enrichmentScore"
           ...
           xlab="",ylab="Normalized enrichment score [arb. unit]")+
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.1)))+
...
for(i in 1:nrow(my_tmp)){
  ...
  p_out<-grid.arrange(p,blank,blank,
           # nrow=2,
           layout_matrix=my_lo,
           heights=c(my_h),
           widths=c(my_w))
  if(my_tmp$Dataset[i]=="Env"){
    #TODO ggsave(file="FigS6_Env.svg",p_out,path=my_supplfigout,dpi = 300, width = 7, height = 7.5,bg=NULL)
  }
  if(my_tmp$Dataset[i]=="Evo"){
    #TODO ggsave(file="FigS6_Evo.svg",p_out,path=my_supplfigout,dpi = 300, width = 7, height = 7.5,bg=NULL)
  }
  if(my_tmp$Dataset[i]=="Mut"){
    #TODO ggsave(file="FigS6_Mut.svg",p_out,path=my_supplfigout,dpi = 300, width = 7, height = 7.5,bg=NULL)
  }
}
...
p
#TODO ggsave(file="Fig3a_Env.svg", path=my_figout,dpi = 300, width = 6, height = 3.5)

q<-ggpar(p,legend = "top",legend.title = list("Gene ratio","q-value"),font.ytickslab= c(10))
leg <- ggpubr::get_legend(q)
as_ggplot(leg)
#TODO ggsave(file="Fig3a_legend.svg", path=my_figout,dpi = 300, width = 6, height = 3.5)
...
p
#TODO ggsave(file="FigS7a.svg", path=my_supplfigout,dpi = 300, width = 6, height = 3.5)
...
p
#TODO ggsave(file="FigS7b.svg", path=my_supplfigout,dpi = 300, width = 6, height = 3.5)
...
plot(v,col = c(mydscolpal[2],mydscolpal[1],mydscolpal[3]), quantities = TRUE) # Evo(red), Env(blue), Mut(green)
...
Sys.time()
```
## 7.6. Calc. probability of overlap
```{r All_GSEA_TRN,warning=FALSE, message=FALSE,fig.height=4,fig.width=2.5,dpi=300}
n_pool<-dTRNsub_stat[(dTRNsub_stat$size>4)&(dTRNsub_stat$size<1001),] %>% nrow()
my_pool<-paste("gene",seq(1,n_pool),sep="_")
trial<-10000
## TRhighs
n_draw_1<-dgsea_trn_env[(dgsea_trn_env$qvalue<0.05)&(dgsea_trn_env$NES>0),] %>% nrow()
n_draw_2<-dgsea_trn_evo[(dgsea_trn_evo$qvalue<0.05)&(dgsea_trn_evo$NES>0),] %>% nrow()
n_draw_3<-dgsea_trn_mut[(dgsea_trn_mut$qvalue<0.05)&(dgsea_trn_mut$NES>0),] %>% nrow()

my_df<-data.frame(trial=seq(1,trial),A=NA,B=NA,C=NA,AB=NA,BC=NA,CA=NA,ABC=NA)

for(i in 1:nrow(my_df)){
  my_tmp1<-my_pool[sample(seq(1,length(my_pool)),size = n_draw_1,replace = F)]
  my_tmp2<-my_pool[sample(seq(1,length(my_pool)),size = n_draw_2,replace = F)]
  my_tmp3<-my_pool[sample(seq(1,length(my_pool)),size = n_draw_3,replace = F)]
  my_df$A[i]<-my_tmp1 %>% paste(collapse = ";")
  my_df$B[i]<-my_tmp2 %>% paste(collapse = ";")
  my_df$C[i]<-my_tmp3 %>% paste(collapse = ";")
  my_df$AB[i]<-my_tmp1[my_tmp1 %in% my_tmp2] %>% length()
  my_df$BC[i]<-my_tmp2[my_tmp2 %in% my_tmp3] %>% length()
  my_df$CA[i]<-my_tmp3[my_tmp3 %in% my_tmp1] %>% length()
  my_df$ABC[i]<-my_tmp1[(my_tmp1 %in% my_tmp2)&(my_tmp1 %in% my_tmp3)] %>% length()
}

nrow(my_df[my_df$ABC>c(length(TRcom)-1),])/nrow(my_df)
mean(my_df$ABC)
...
nrow(my_df[my_df$ABC>0,])/nrow(my_df)
mean(my_df$ABC)
...
#TODO ggsave(file="Fig3d_TRcom.svg", path=my_figout,dpi = 300, width = 4.5, height = 3.5)
...
#TODO ggsave(file="Fig3d_Non_TRcom.svg", path=my_figout,dpi = 300, width = 7, height = 6)
...
p<-gghistogram(df.rndmpick.edgedens,x="test",y="..density..",bins=30,
            ...
            xlab="Edge density",ylab="Relative frequency [arb. unit]")+
...
temp<- c(mycolpal[2],"gray70","gray20","white")
...
df.edge.dens$showdot<-df.edge.dens$EdgeDensity
df.edge.dens[df.edge.dens$Class=="Random",]$showdot<-NA
...
p
p<-ggbarplot(df.edge.dens,x="Class2",y="EdgeDensity",fill="Class2",palette = temp,xlab="",ylab="Edge density",legend.title="",legend="")+
  geom_dotplot(data =df.edge.dens,aes(x=factor(Class2),y=showdot),color="black",alpha=0.8,binaxis = "y",dotsize = 1,stackdir = "center")+
  geom_linerange(aes(ymax=High_sd,ymin=Low_sd),width=0.5)+
  scale_y_continuous(breaks = get_breaks(by = 0.1, from = 0),limits =  c(0, 0.22))+rotate_x_text(90)+
  scale_x_discrete(labels = my_labs)
p<-ggpar(p,font.y=c(17),font.xtickslab = c(17),font.ytickslab = c(17),xlab=FALSE)
p
#TODO ggsave(file="Fig3e.svg", path=my_figout,dpi = 300, width = 2.5, height = 5)
Sys.time()
```
...
Sys.time()
```
## 8.11. The relationship between the number of regulatorion and DM values
### 8.11.1. TRcom-containing or TRcom-less
...
p0<-ggbarplot(figtemp1.2,x="Count.allregs",y="DMenv",fill="TRcomflag2",width=0.5,size=0,
   ...
   ylab(expression(DM[env]~"[arb. unit]"))
p0<-ggpar(p0,legend="")+
  geom_text(aes(label=label,x=Count.allregs,y=0.5,fill=TRcomflag2,color=TRcomflag2),position=position_dodge(width=1),angle=90,size=4.5)+
  scale_y_continuous(limits=c(0.4,0.6))+
  clean_theme()
p0
#TODO ggsave(file="Fig4a_count.svg", path=my_figout,dpi = 300, width = 6, height = 6)
...
lay<-t(rbind(seq(1,5)))
blank <- grid.rect(gp=gpar(col="white"))
p<-grid.arrange(as_ggplot(leg),blank,figlist[[1]],figlist[[2]],figlist[[3]],
             layout_matrix=lay,
             bottom=textGrob("Number of unique regulators",gp=gpar(fontsize=16)),
             heights=c(0.2,0.1,rep(1.1,3)))
#TODO ggsave(file="Fig4a.svg",p, path=my_figout,dpi = 300, width = 6, height = 6)
...
my_df<-data.frame(Nb_uniqTRcom=seq(0,max(figtemp3$Count.TRcom)),Count=0,label=NA)
for(i in 1:nrow(my_df)){
  my_df$Count[i]<-figtemp3[figtemp3$Count.TRcom==my_df$Nb_uniqTRcom[i],] %>% nrow()
}
my_df$label<-my_df$Count %>% as.character()
my_df$y<-1
my_df$flag<-"TRcom"
my_df[my_df$Nb_uniqTRcom==0,]$flag<-"Null"
my_df$flag<-factor(my_df$flag,levels = qw(Null,TRcom))
p0<-ggbarplot(my_df,x="Nb_uniqTRcom",y="y",fill="flag",width=0.5,size=0,
              ylim=c(0,2),
              palette = c("gray50",mycolpal[2]),
              position = position_dodge(width=0.7))+
   ylab(expression(DM[env]~"[arb. unit]"))
p0<-ggpar(p0,legend="")+
  geom_text(aes(label=label,x=Nb_uniqTRcom,y=1.5,fill=flag,color=flag),position=position_dodge(width=1),angle=90,size=4.5)+
  scale_y_continuous(limits=c(1.4,1.6))+
  clean_theme()
p0
#TODO ggsave(file="Fig4b_count.svg", path=my_figout,dpi = 300, width = 4, height = 5)
...
lay<-t(rbind(seq(1,2)))
p<-grid.arrange(p4,p5,
             layout_matrix=lay,
             heights=c(0.7,1))
#TODO ggsave(file="Fig4bc.svg",p, path=my_figout,dpi = 300, width = 4, height = 5)
...
Sys.time()
```
## 8.14. A dummy data only for presentating principle of GSEA
```{r TRN,warning=FALSE, message=FALSE,fig.height=3,fig.width=12,dpi=300}
temp<-dBnb.comp %>% select(Bnb,gene)
my_df<-left_join(df.regcount,temp,by=c("gene_id"="Bnb"))
my_df<-my_df %>% select(gene,DMenv,DMevo,DMmut,regulators)
my_df<-my_df[order(my_df$DMenv,decreasing = T),]
rownames(my_df)<-seq(1,nrow(my_df))
my_regs<-qw(FlhDC,DksA)
my_rg<-my_regs[1]
my_df$reg_example<-ifelse(str_detect(pattern = my_rg,string = my_df$regulators),1,-1)
my_df[is.na(my_df$regulators),]$reg_example<--1
my_df$label<-ifelse(my_df$reg_example==1,my_rg,"Others")
my_df$label<-factor(my_df$label,levels=c(my_rg,"Others"))
my_nh<-nrow(my_df[my_df$reg_example==1,])
my_nmiss<-nrow(my_df[my_df$reg_example==-1,])


my_df$reg_rand<-rbinom(n = nrow(my_df),1,prob = my_nh/nrow(my_df))
my_df[my_df$reg_rand==0,]$reg_rand<--1
my_df$label_rand<-ifelse(my_df$reg_rand==1,"Random","Others")
my_df$label_rand<-factor(my_df$label_rand,levels=c("Random","Others"))

my_df$Size_exampl<-ifelse(my_df$reg_example==1,0.5,0.1)

p1e<-ggbarplot(my_df,x="gene",y="DMenv",size=1,fill="label",color="label",palette = c(mycolpal[2],"gray80"),legend="")+ # #E46726
  rotate_x_text(angle=90)+ylab(expression(DM[env]))
p1e<-ggpar(p1e,font.y=c(30),font.ytickslab = c(30))
p1e<-p1e+theme(axis.text.x= element_blank(),axis.title.x=element_blank(),axis.ticks.x = element_blank())
p1e

p1m<-ggbarplot(my_df[order(my_df$DMmut,decreasing = T),],x="gene",y="DMmut",size=1,fill="label",color="label",palette = c(mycolpal[2],"gray80"),legend="")+ # #E46726
  rotate_x_text(angle=90)+ylab(expression(DM[mut]))
p1m<-ggpar(p1m,font.y=c(30),font.ytickslab = c(30))
p1m<-p1m+theme(axis.text.x= element_blank(),axis.title.x=element_blank(),axis.ticks.x = element_blank())
p1m


p2<-ggbarplot(my_df,x="gene",y="DMenv",size=0.5,fill="label_rand",color="label_rand",palette = c("black","gray80"),legend="")+ #"#619CFF" #E46726
  rotate_x_text(angle=90)+ylab(expression(DM[env]))
p2<-ggpar(p2,font.y=c(30),font.ytickslab = c(30))
...
kr<-0
for(i in 1:nrow(my_df)){
  if(my_df$reg_example[i]==1){
    k<-k+1/my_nh
  }else{
    k<-k-1/my_nmiss
  }
  my_df$RunES[i]<-k
  if(my_df$reg_rand[i]==1){
    kr<-kr+1/my_nh
  }else{
    kr<-kr-1/my_nmiss
  }
  my_df$RunES_rand[i]<-kr
}


p3<-ggline(my_df,x="order",y="RunES",plot_type = "l",ylab="Enrichment score",size=2)+
  scale_y_continuous(breaks = get_breaks(by = 0.2, from = -0.2),limits =  c(-0.2, 0.45))+
  geom_hline(yintercept = 0)
p3<-ggpar(p3,font.y=c(26),font.ytickslab = c(26))
p3<-p3+theme(axis.text.x= element_blank(),axis.title.x=element_blank(),axis.ticks.x = element_blank())
p3

p4<-ggline(my_df,x="order",y="RunES_rand",plot_type = "l",ylab="Enrichment score",size=2)+
  scale_y_continuous(breaks = get_breaks(by = 0.2, from = -0.2),limits =  c(-0.2, 0.45))+
  geom_hline(yintercept = 0)
p4<-ggpar(p4,font.y=c(26),font.ytickslab = c(26))
...
p<-grid.arrange(p1,p2,p3,widths=c(1,1,1),nrow=1)
#TODO ggsave(file="Fig6a_Env.svg",p, path=my_figout,dpi = 300, width = 7.5, height = 2.5)
#TODO ggsave(file="FigS8a_Env.svg",p, path=my_supplfigout,dpi = 300, width = 7.5, height = 2.5)
Sys.time()
```
...
p<-grid.arrange(p1,p2,p3,widths=c(1,1,1),nrow=1)
#TODO ggsave(file="FigS8b.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)
Sys.time()
```
...
p<-ggline(figtemp3,x="PC",y="Fraction_pct",numeric.x.axis = T,color="Source",plot_type = "l",
       legend.title="",legend="",
       # palette = temp,
       palette = c(mydscolpal,rep(mydscolpal[3],3)),
       linetype = "Source",
...
leg <- ggpubr::get_legend(q)
as_ggplot(leg)
#TODO ggsave(file="Fig6c_legend.svg", path=my_figout,dpi = 300, width = 2.5, height = 2.5)
...
p1<-ggline(figtemp3[figtemp3$Source %in% qw(Env,Evo,Mut),],x="PC",y="Fraction_pct",numeric.x.axis = T,color="Source",plot_type = "l",
  ...
  scale_y_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,100))
p1
#TODO ggsave(file="Fig6c.svg", path=my_figout,dpi = 300, width = 2.5, height = 2.5)
...
ind.p
#TODO ggsave(file="Fig6a_Evo.svg",ind.p, path=my_figout,dpi = 300, width = 2.5, height = 2.5)
...
leg <- ggpubr::get_legend(q)
as_ggplot(leg)
#TODO ggsave(file="Fig6a_Evo_legend.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,0.3),nrow=1)
#TODO ggsave(file="FigS9.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)
Sys.time()
```
...
ind.p
#TODO ggsave(file="Fig6a_Mut.svg",ind.p, path=my_figout,dpi = 300, width = 2.5, height = 2.5)

q<-ggpar(ind.p,legend = "right",legend.title="",font.legend = c(8))
leg <- ggpubr::get_legend(q)
as_ggplot(leg)
#TODO ggsave(file="Fig6a_Mut_legend.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,0.4),nrow=1)
#TODO ggsave(file="FigS10a.svg",p, path=my_supplfigout,dpi = 300, width = 10, height = 3)
Sys.time()
```
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,0.4),nrow=1)
#TODO ggsave(file="FigS10b.svg",p, path=my_supplfigout,dpi = 300, width = 10, height = 3)
Sys.time()
```
...
p<-grid.arrange(p4,p5,p6,as_ggplot(leg2),widths=c(1,1,1,0.4),nrow=1)
#TODO ggsave(file="Fig6b.svg",p, path=my_figout,dpi = 300, width = 11, height = 4.5)
...
```{r PCA_Alignment,warning=FALSE, message=FALSE,fig.height=4.5,fig.width=11,dpi=300}
...
### 9.6.1. perform GSEA
```{r PCA_GSEA_TRcom,warning=FALSE, message=FALSE,fig.height=6,fig.width=6,dpi=300}
TRN_TRcom<-TRN
...
for (j in 1:length(my_ds)){
  ...
  grid.arrange(figlist4[[1]],figlist4[[2]],figlist4[[3]],as_ggplot(leg), #blank,
               widths=c(rep(1,4),1),
               left=textGrob("Normalized enrichment score [arb. unit]", rot=90,gp=gpar(fontsize=20)),
               layout_matrix=lay)
}
Sys.time()
```
...
PC1~PC3
```{r PCA_GSEA_TRcom,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# venn
venn_all<-list(
...
Two regulatory modes (activation or inhibition) were distinguished.
### 9.7.1. perform GSEA
```{r PCA_GSEA_allTR,warning=FALSE, message=FALSE,fig.height=7,fig.width=14,dpi=300}
TRN_TRall<-TRN
...
for (j in 1:length(my_ds)){
  ...
  # lay<-rbind(c(1,1,1,1,4),c(2,2,5,5,4),c(3,3,3,3,4))
  lay<-rbind(c(1,1,1,1,4),c(2,2,2,2,4),c(3,3,3,3,4))
  p<-grid.arrange(figlist4[[1]],figlist4[[2]],figlist4[[3]],as_ggplot(leg), #blank,
               widths=c(rep(1,4),0.5),
               left=textGrob("Normalized enrichment score [arb. unit]", rot=90,gp=gpar(fontsize=24)),
               layout_matrix=lay)
  p
  if(my_ds[j]=="Env"){
    #TODO ggsave(file="Fig6d.svg",p, path=my_figout,dpi = 300, width = 14, height = 7)
  }
  if(my_ds[j]=="Evo"){
    #TODO ggsave(file="FigS12a.svg",p, path=my_supplfigout,dpi = 300, width = 14, height = 7)
  }
  if(my_ds[j]=="Mut"){
    #TODO ggsave(file="FigS12b.svg",p, path=my_supplfigout,dpi = 300, width = 14, height = 7)
  }
}
...
PC1~PC3
```{r PCA_GSEA_allTR,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# venn
venn_all<-list(
...
Sys.time()
```
### 9.7.4. Calc Prob of overlap
PC1~PC3
```{r PCA_GSEA_TRN,warning=FALSE, message=FALSE,fig.height=4,fig.width=2.5,dpi=300}
n_pool<-dTRN_TRall$reg_eff %>% unique() %>% length()
my_pool<-paste("gene",seq(1,n_pool),sep="_")
trial<-10000
## TRhighs
n_draw_1<-dTRall_pca_gsea[dTRall_pca_gsea$dataset=="Env",]$ID %>% unique() %>% length()
n_draw_2< dTRall_pca_gsea[dTRall_pca_gsea$dataset=="Evo",]$ID %>% unique() %>% length()
n_draw_3<-dTRall_pca_gsea[dTRall_pca_gsea$dataset=="Mut",]$ID %>% unique() %>% length()
...
nrow(my_df[my_df$ABC>31,])/nrow(my_df)
mean(my_df$ABC)
...
```
### 9.7.5. Venn diagram of enriched regulation in PC1
PC1 only
...
```
# 10. Gene Ontology
## 10.1. Create a custom GO slim dataset (a subset of GO, dfgoslim2, dfgoslim23)
...
```{r GO_slim,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
...
p<-ggscatter(df_DMall_gocat,x="DMenv",y="DMmut",
          ...
          color="Domain",palette = mygocolpal[1:3],
          cor.coef = T,
...
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.xtickslab = my_fontsize,font.ytickslab = my_fontsize)
p
#TODO ggsave(file="Fig2b_EnvMut.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.xtickslab = my_fontsize,font.ytickslab = my_fontsize)
p
#TODO ggsave(file="Fig2b_EnvEvo.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
p<-ggpar(p,font.y=my_fontsize,font.x=my_fontsize,font.xtickslab = my_fontsize,font.ytickslab = my_fontsize)
p
#TODO ggsave(file="Fig2b_MutEvo.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
p<-ggscatter(df_DMall_gocat,x="DMenv",y="DMmut",
          ...
          color="Category",palette = mygocolpal[1:3],
          cor.coef = T,
          cor.coeff.args = list(method = "spearman",label.sep = "\n",size=5,label.x=-0.1,label.y=0.6),legend="")+
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.3)))+
  xlab(expression("Mean"~DM[env]~"[arb. unit]"))+
...
leg <- ggpubr::get_legend(q)
as_ggplot(leg)
#TODO ggsave(file="Fig2b_legend.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
ggtexttable(my_tmp,rows=NULL)
#TODO ggsave(file="Fig2b_table.svg", path=my_figout,dpi = 300, width = 10, height = 2.5)
Sys.time()
```
### 10.3.2. all 
...
for(i in 1:nrow(my_tmp)){
  ...
  p_out<-grid.arrange(p,blank,blank,
           ...
           widths=c(my_w))
  if(my_tmp$Dataset[i]=="Env"){
    #TODO ggsave(file="FigS4_Env.svg", p_out,path=my_supplfigout,dpi = 300, width = 7, height = 10,bg=NULL)
  }
  if(my_tmp$Dataset[i]=="Evo"){
    #TODO ggsave(file="FigS4_Evo.svg", p_out,path=my_supplfigout,dpi = 300, width = 7, height = 10,bg=NULL)
  }
  if(my_tmp$Dataset[i]=="Mut"){
    #TODO ggsave(file="FigS4_Mut.svg", p_out,path=my_supplfigout,dpi = 300, width = 7, height = 10,bg=NULL)
  }
}
...
Sys.time()
```
### 10.4.6. Calc probability of overlap
```{r GO_slim,warning=FALSE, message=FALSE,fig.height=4,fig.width=2.5,dpi=300}
n_pool<-dfgoslim3$go %>% unique() %>% length()
my_pool<-paste("gene",seq(1,n_pool),sep="_")
trial<-10000
## DMhigh
n_draw_1<-dgsea_go_env_all[(dgsea_go_env_all$NES>0)&(dgsea_go_env_all$qvalue<0.05),] %>% nrow()
n_draw_2<-dgsea_go_evo_all[(dgsea_go_evo_all$NES>0)&(dgsea_go_evo_all$qvalue<0.05),] %>% nrow()
n_draw_3<-dgsea_go_mut_all[(dgsea_go_mut_all$NES>0)&(dgsea_go_mut_all$qvalue<0.05),] %>% nrow()
...
nrow(my_df[my_df$AB>3&my_df$BC>3&my_df$CA>3,])/nrow(my_df)
...
nrow(my_df[my_df$ABC>7,])/nrow(my_df)
mean(my_df$ABC)
...
for(ii in 1:length(mypcs)){
  ...
  p_out<-grid.arrange(p,blank,blank,
           ...
           widths=c(my_w))
  if(k== "PC1"){
    #TODO ggsave(file="FigS14_PC1.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
  if(k== "PC2"){
    #TODO ggsave(file="FigS14_PC2.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
  if(k== "PC3"){
    #TODO ggsave(file="FigS14_PC3.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
}
Sys.time()
```
...
for(ii in 1:length(mypcs)){
  ...
  p_out<-grid.arrange(p,blank,blank,
           ...
           widths=c(my_w))
  if(k== "PC1"){
    #TODO ggsave(file="FigS15_PC1.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
  if(k== "PC2"){
    #TODO ggsave(file="FigS15_PC2.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
  if(k== "PC3"){
    #TODO ggsave(file="FigS15_PC3.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
}
Sys.time()
```
...
for(ii in 1:length(mypcs)){
  ...
  p_out<-grid.arrange(p,blank,blank,
           ...
           widths=c(my_w))
  if(k== "PC1"){
    #TODO ggsave(file="FigS16_PC1.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
  if(k== "PC2"){
    #TODO ggsave(file="FigS16_PC2.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
  if(k== "PC3"){
    #TODO ggsave(file="FigS16_PC3.svg",p_out, path=my_supplfigout,dpi = 300, width = 10, height = 10)
  }
}
Sys.time()
```
...
p<-grid.arrange(as_ggplot(leg),p1,p2,widths=c(0.6,0.65),heights=c(0.1,0.9),layout_matrix=my_lo)
#TODO ggsave(file="Fig6g.svg",p, path=my_figout,dpi = 300, width = 4.5, height = 6)
Sys.time()
```
...
#TODO ggsave(file="Fig6f.svg", path=my_figout,dpi = 300, width = 10, height = 4)
Sys.time()
```
# 11. The Rwr dataset
Baumstark 2015 Nature communications
https://www.nature.com/articles/ncomms10105#accession-codes
Strain: TOP10
Supplementary data: Transcriptome data of rewired strains. The log2-transformed expression levels
## 11.1. Load data
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
Bstkdat<-as.data.frame(read_csv( paste(getwd(),"Rwr_DataSet","probe_set_expression_value_norm_MG1655.csv",sep="/"))) # all data
Bstk_probe<-as.data.frame(read_csv( paste(getwd(),"Rwr_DataSet","ProbeInfo.csv",sep="/"))) # probe infomation
Bstk_sampleinfo<-as.data.frame(read_csv( paste(getwd(),"Rwr_DataSet","SampleInfo.csv",sep="/"))) # probe infomation
colnames(Bstkdat)<-c("probe_id",Bstk_sampleinfo$Name)
Bstkdat<-left_join(Bstkdat,Bstk_probe[,which(colnames(Bstk_probe) %in% qw(Bnb,probe_id))])
Bstkdat<-Bstkdat[!is.na(Bstkdat$Bnb),]
Bstkdat<-Bstkdat[Bstkdat$Bnb %in% dBnb.comp$Bnb,]

Bstkdat<-Bstkdat[,which(colnames(Bstkdat) %nin% "probe_id")]
temp<-normalize.quantiles(as.matrix(Bstkdat[,which(colnames(Bstkdat) %nin% "Bnb")]))
temp<-as.data.frame(temp)
colnames(temp)<-colnames(Bstkdat[,which(colnames(Bstkdat) %nin% "Bnb")])
Bstkdat<-cbind(Bstkdat$Bnb,temp)
colnames(Bstkdat)[1]<-"Bnb"

ggscatter(Bstkdat,x="Co_rep1",y="Co_rep2",size=0.5,alpha=0.2)
ggscatter(Bstkdat,x="Co_rep1",y="flhD_0_rep1",size=0.5,alpha=0.2)
Sys.time()
```
## 11.2. Check reproducibility
## 11.2.1.  Correlation analysis to filter lowly-reproducible data out
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
Bstkdat_cor<-Bstk_sampleinfo
Bstkdat_cor$cor<-NA
my_col<-Bstkdat_cor$Prom_ORF %>% unique()
i<-1
for(i in 1:length(my_col)){
  my_df<-Bstkdat %>% select(Bstk_sampleinfo[Bstk_sampleinfo$Prom_ORF==my_col[i],]$Name)
  my_res<-my_df %>% cor_test(method = "spearman")
  Bstkdat_cor[Bstkdat_cor$Prom_ORF==my_col[i],]$cor<-my_res[my_res$var1!=my_res$var2,]$cor %>% mean()
}

Bstkdat_cor<-Bstkdat_cor %>% distinct(Prom_ORF,.keep_all = T)

gghistogram(Bstkdat_cor,x="cor",fill = "gray")+
  xlab("Spearman's R")


Sys.time()
```
## 11.2.2. Relevance of the criteria of filtration (preparation)
```{r Rwr,fig.height=3,fig.width=3.8}
my_gen<-2500 # number of genes
my_rep<-9 # number of replicates
my_sigma<-diag(my_rep)
for(i in 1:nrow(my_sigma)){
  for(j in 1:ncol(my_sigma)){
    if(my_sigma[i,j]!=1){
      my_sigma[i,j]<-0.95
    }
  }
}
my_df<-as.data.frame(mvrnorm(n=my_gen, mu=rep(1,my_rep), Sigma=my_sigma))
colnames(my_df)<-paste("rep",seq(1,my_rep),sep="_")
ggscatter(my_df,x="rep_1",y="rep_2",cor.coef = T,cor.method = "spearman")


my_df2<-createEmptyDf(nrow=my_gen,ncol=3,colnames = paste("rep",qw(A,B,C),sep="_"))
k<-0
for(i in 1:3){
  my_df2[,i]<-my_df[,c(k+1,k+2,k+3)] %>%rowMeans()
  k<-k+3
}
ggscatter(my_df2,x="rep_A",y="rep_B",cor.coef = T,cor.method = "spearman")


```
## 11.2.3. Relevance of the criteria of filtration
```{r Rwr,fig.height=3,fig.width=3}
my_gen<-2442 # number of genes
my_line<-8
my_rep<-3 # number of replicates per line
my_sigma<-diag(my_rep*my_line)
for(i in 1:ncol(my_sigma)){
  my_sigma[my_sigma[,i]!=1,i]<-0.95
}
my_df<-as.data.frame(mvrnorm(n=my_gen, mu=rep(0,my_rep*my_line), Sigma=my_sigma))
colnames(my_df)<-paste("line",rep(seq(1,my_line),each=my_rep),"rep",rep(rep(seq(1,my_rep)),my_line),sep="_")
my_line_uniq<-paste("line",seq(1,my_line),sep="_")
my_df2<-createEmptyDf(nrow=my_gen,ncol=my_line,colnames = my_line_uniq)
k<-0
i<-1
for(i in 1:my_line){
  my_col<-colnames(my_df)[grepl(pattern = my_line_uniq[i],x=colnames(my_df))]
  my_df2[,i]<-my_df[,which(colnames(my_df) %in% my_col)] %>%rowMeans()
  k<-k+my_rep
}


my_res<-my_df2 %>% cor_test(method = "spearman")
my_res[my_res$var1!=my_res$var2,]$cor %>% mean() 


ggscatter(my_df,x="line_1_rep_1",y="line_1_rep_2",cor.coef = T,cor.method = "spearman",size=0.2,alpha=0.2)

ggscatter(my_df2,x="line_1",y="line_5",cor.coef = T,cor.method = "spearman",size=0.2,alpha=0.2)


```
## 11.3. Use all genes
### 11.3.1. Calc DM
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=3.5,dpi=300}
temp3<-Bstkdat[,which(colnames(Bstkdat) %nin% "Bnb")]
...
Bstkdat<-cbind(Bstkdat,temp5)

Bstkdat<-Bstkdat[order(Bstkdat$mean),] # sorting 
Bstkdat<-transform(Bstkdat,RunmedVar=runmed(Bstkdat$sd,41),lmg=Bstkdat$mean)
lmgn<-data.frame(x=Bstkdat$lmg,y=Bstkdat$RunmedVar)
sp<-smooth.spline(lmgn,df=12)
x<-seq(min(lmgn$x),max(lmgn$x),length=1000)
pred<-as.data.frame(x)
pred<-transform(pred,smsp=predict(sp,x)[2])
...
pred2<-transform(pred2,y=predict(sp,pred2)[2])
colnames(pred2)<-c("x","smoothsp")
Bstkdat<-transform(Bstkdat,SmoothSpline=pred2$smoothsp)
Bstkdat<-transform(Bstkdat,DMrw=Bstkdat$sd-Bstkdat$SmoothSpline) # calculate distance from smooth spline to SD 

df_rw_bstk<-left_join(df_DMall,Bstkdat,by="Bnb")

## Running median
# mycolpal<-get_palette("lancet",2)
ggscatter(df_rw_bstk,x="mean",y="sd",
          cor.coef = TRUE,cor.coeff.args = list(method = "spearman", label.x = 5, label.y=1, label.sep = "\n",size=4),
          alpha = 0.2, size=0.3,legend="")+
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
  ylab(expression("Standard deviation [arb. unit]"))+
  geom_line(data=df_rw_bstk,aes(x=mean,y=RunmedVar), colour="darkorange", size=0.5)+
  geom_line(data=pred,aes(x=x,y=smoothsp), colour="deepskyblue", size=0.5) # "deepskyblue"

ggscatter(df_rw_bstk,x="mean",y="DMrw",
          cor.coef = TRUE, # Add correlation coefficient. see ?stat_cor
          cor.coeff.args = list(method = "spearman", label.x = 5, label.y=1, label.sep = "\n",size=4),
          alpha = 0.2, size=0.3,legend="")+
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
  ylab(expression(DM[rw]~"[arb. unit]"))



Sys.time()
```
### 11.3.2. Comprison of DMs
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=9,dpi=300}
# DMenv vs. DMrw
p1<-ggscatter(df_rw_bstk,x="DMenv",y="DMrw",
...
temp0<-temp0[order(temp0$Bnb,decreasing = F),]
rownames(temp0)<-temp0$Bnb
temp0<-na.omit(temp0[,which(colnames(temp0) %in% Bstk_sampleinfo$Name)])
temp0<-t(temp0) # transpose the matrix so genes are as columns
figtemp<-as.data.frame(temp0)
figtemp<-figtemp %>% select_if(negate(anyNA))
res.pca.global_rw <- prcomp(figtemp, scale. = T)
# The PC scores are stored in the "x" value of the prcomp object
pc_scores_rw <- res.pca.global_rw$x %>% 
  # convert to a tibble retaining the sample names as a new column
  as_tibble(rownames = "Condition")
pc_scores_rw$label<-rownames(pc_scores_rw)

pc_eigenvalues_rw <- res.pca.global_rw$sdev^2
pc_eigenvalues_rw <- tibble(PC = factor(1:length(pc_eigenvalues_rw)), 
                         variance = pc_eigenvalues_rw) %>%   
  mutate(pct = variance/sum(variance)*100) %>% # add a new column with the percent variance
  mutate(pct_cum = cumsum(pct)) # add another column with the cumulative variance explained

# Scree plot
figtemp1<-pc_eigenvalues_rw
figtemp1$PC<-as.numeric(figtemp1$PC)
dscree_rw<-figtemp1
ggline(figtemp1,x="PC",y="pct_cum",numeric.x.axis = T,plot_type = "l",
        color="gray20",
...
# Variance explatind by PC1~PCk
k<-3
print(paste("PC1~PC",k, " explains ", pc_eigenvalues_rw$pct_cum[k] ,"% variance",sep=""))


Sys.time()
```
### 11.3.4. 2D plot (PC1~PC3)
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=9,dpi=300}
figtemp<-pc_scores_rw
figtemp$label<-figtemp$Condition
figtemp$label<-figtemp$label %>% str_remove_all(pattern = "_rep[0-9]$")
figtemp$label<-factor(figtemp$label,levels = unique(figtemp$label))
...
p1<-ggscatter(data=figtemp,x="PC1",y="PC2",
  ...
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p1<-ggpar(p1,
      xlab = paste("PC1 (", round(pc_eigenvalues_rw$pct[1],digits = 0),"%)",sep=""),
      ylab = paste("PC2 (", round(pc_eigenvalues_rw$pct[2],digits = 0),"%)",sep=""))
...
p2<-ggscatter(data=figtemp,x="PC2",y="PC3",
  ...
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p2<-ggpar(p2,
      xlab = paste("PC2 (", round(pc_eigenvalues_rw$pct[2],digits = 0),"%)",sep=""),
      ylab = paste("PC3 (", round(pc_eigenvalues_rw$pct[3],digits = 0),"%)",sep=""))
...
Sys.time()
```
### 11.3.5. Correlation between rotation (eigenvector) for PC1~PC3
```{r Rwr,warning=FALSE, message=FALSE,fig.height=4.5,fig.width=11,dpi=300}
mypcs<-paste("PC",seq(1:3),sep="")
my_lim<-c(0,0.65)
# Env vs. Rw
temp5<-data.frame(Env_PC=rep(seq(1,3),each=3),Rw_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp5)){
  temp1<-as.data.frame(res.pca.global_env$rotation[,temp5$Env_PC[i]])
  colnames(temp1)<-"rot.env"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(res.pca.global_rw$rotation[,temp5$Rw_PC[i]])
  colnames(temp2)<-"rot.rw"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp5$Cor[i]<-cor.test(temp4$rot.env,temp4$rot.rw, method = "spearman")$estimate
  temp5$p.value[i]<-cor.test(temp4$rot.env,temp4$rot.rw, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp5.1)){
  for(j in 1:ncol(temp5.1)){
    temp5.1[i,j]<-temp5[temp5$Env_PC==i&temp5$Rw_PC==j,]$Cor
    temp5.2[i,j]<-temp5[temp5$Env_PC==i&temp5$Rw_PC==j,]$p.value
  }
}
temp5.3<-abs(temp5.1)

# Evo vs. Rw
temp6<-data.frame(Evo_PC=rep(seq(1,3),each=3),Rw_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp6)){
  temp1<-as.data.frame(res.pca.global_evo$rotation[,temp6$Evo_PC[i]])
  colnames(temp1)<-"rot.evo"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(res.pca.global_rw$rotation[,temp6$Rw_PC[i]])
  colnames(temp2)<-"rot.rw"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp6$Cor[i]<-cor.test(temp4$rot.evo,temp4$rot.rw, method = "spearman")$estimate
  temp6$p.value[i]<-cor.test(temp4$rot.evo,temp4$rot.rw, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp6.1)){
  for(j in 1:ncol(temp6.1)){
    temp6.1[i,j]<-temp6[temp6$Evo_PC==i&temp6$Rw_PC==j,]$Cor
    temp6.2[i,j]<-temp6[temp6$Evo_PC==i&temp6$Rw_PC==j,]$p.value
  }
}
temp6.3<-abs(temp6.1)

# Mut vs. Rw
temp7<-data.frame(Mut_PC=rep(seq(1,3),each=3),Rw_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp7)){
  temp1<-as.data.frame(res.pca.global_mut$rotation[,temp7$Mut_PC[i]])
  colnames(temp1)<-"rot.mut"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(res.pca.global_rw$rotation[,temp7$Rw_PC[i]])
  colnames(temp2)<-"rot.rw"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp7$Cor[i]<-cor.test(temp4$rot.mut,temp4$rot.rw, method = "spearman")$estimate
  temp7$p.value[i]<-cor.test(temp4$rot.mut,temp4$rot.rw, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp7.1)){
  for(j in 1:ncol(temp7.1)){
    temp7.1[i,j]<-temp7[temp7$Mut_PC==i&temp7$Rw_PC==j,]$Cor
    temp7.2[i,j]<-temp7[temp7$Mut_PC==i&temp7$Rw_PC==j,]$p.value
  }
}
temp7.3<-abs(temp7.1)
...
Sys.time()
```
## 11.4. Use genes that are present in the main three datasets
### 11.4.1. Create averaged data
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
Bstkdat_av<-createEmptyDf(nrow = nrow(Bstkdat),ncol=(length(unique(Bstk_sampleinfo$Prom_ORF))+1),colname=c("Bnb",unique(Bstk_sampleinfo$Prom_ORF)))
Bstkdat_av$Bnb<-Bstkdat$Bnb
i<-2
for(i in 2:ncol(Bstkdat_av)){
  my_col<-Bstk_sampleinfo[Bstk_sampleinfo$Prom_ORF==colnames(Bstkdat_av)[i],]$Name
  my_df<-Bstkdat %>% select(my_col)
  Bstkdat_av[,i]<-my_df %>% as.matrix() %>% rowMeans()
}

# Use high quality data
Bstkdat_av <- Bstkdat_av %>% select(Bnb,Bstkdat_cor[Bstkdat_cor$cor>=0.95,]$Prom_ORF)
Bstkdat_av <- Bstkdat_av[order(Bstkdat_av$Bnb,decreasing = F),]
Sys.time()
```
### 11.4.2. Create smaller datasets of Env, Evo and Mut
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
dat_small_env<-log_tpm_pl.pr2[log_tpm_pl.pr2$Bnb %in% Bstkdat_av$Bnb,]
dat_small_env<-dat_small_env[order(dat_small_env$Bnb,decreasing = F),]

dat_small_evo<-dcol16.qnorm[dcol16.qnorm$Bnb %in% Bstkdat_av$Bnb,]
dat_small_evo<-dat_small_evo %>% select(Bnb,colnames(dat_small_evo)[colnames(dat_small_evo) %in% col16.strain])
dat_small_evo<-dat_small_evo[order(dat_small_evo$Bnb,decreasing = F),]

dat_small_mut<-datlog.norm[datlog.norm$Bnb %in% Bstkdat_av$Bnb,]
dat_small_mut<-dat_small_mut %>% select(Bnb,colnames(dat_small_mut)[colnames(dat_small_mut) %in% MAlin.sel])
dat_small_mut<-dat_small_mut[order(dat_small_mut$Bnb,decreasing = F),]

# Scailing
my_mat<-Bstkdat_av %>% select(-Bnb)
rownames(my_mat)<-Bstkdat_av$Bnb
dat_scaled_rwr<-my_mat %>% t() %>% scale()

my_mat<-dat_small_env %>% select(-Bnb)
rownames(my_mat)<-dat_small_env$Bnb
dat_scaled_env<-my_mat %>% t() %>% scale()

my_mat<-dat_small_evo %>% select(-Bnb)
rownames(my_mat)<-dat_small_evo$Bnb
dat_scaled_evo<-my_mat %>% t() %>% scale()

my_mat<-dat_small_mut %>% select(-Bnb)
rownames(my_mat)<-dat_small_mut$Bnb
dat_scaled_mut<-my_mat %>% t() %>% scale()
Sys.time()
```
### 11.4.3. Perform PCA based on Env (smaller) and mapping the other datsets on to the PC space of Env
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=4,dpi=300}
my_res<-prcomp(dat_scaled_env,scale. = T)
my_eign <- my_res$sdev^2
my_eign <- tibble(PC = factor(1:length(my_eign)), 
                         variance_env = my_eign ) %>%   
  mutate(pct_env = variance_env/sum(variance_env)*100) %>% # add a new column with the percent variance
  mutate(pct_cum_env = cumsum(pct_env)) # add another column with the cumulative variance explained
my_eign$PC<-as.numeric(my_eign$PC)

# Calc % of variance of the Env explaind py PCs 
# evo
my_eign$variance_evo<-NA
my_eign$pct_evo<-NA
my_eign$pct_cum_evo<-NA
i<-1
for(i in 1:nrow(my_eign)){
  my_tmp<-dat_scaled_evo %*% my_res$rotation[,i]
  my_eign$variance_evo[i]<-var(my_tmp)
}
my_eign$pct_evo<-my_eign$variance_evo/sum(my_eign$variance_env)*100
my_eign$pct_cum_evo<-cumsum(my_eign$pct_evo)

# mut
my_eign$variance_mut<-NA
my_eign$pct_mut<-NA
my_eign$pct_cum_mut<-NA
i<-1
for(i in 1:nrow(my_eign)){
  my_tmp<-dat_scaled_mut %*% my_res$rotation[,i]
  my_eign$variance_mut[i]<-var(my_tmp)
}
my_eign$pct_mut<-my_eign$variance_mut/sum(my_eign$variance_env)*100
my_eign$pct_cum_mut<-cumsum(my_eign$pct_mut)

# rwr
my_eign$variance_rwr<-NA
my_eign$pct_rwr<-NA
my_eign$pct_cum_rwr<-NA
i<-1
for(i in 1:nrow(my_eign)){
  my_tmp<-dat_scaled_rwr %*% my_res$rotation[,i]
  my_eign$variance_rwr[i]<-var(my_tmp)
}
my_eign$pct_rwr<-my_eign$variance_rwr/sum(my_eign$variance_env)*100
my_eign$pct_cum_rwr<-cumsum(my_eign$pct_rwr)

my_df<-my_eign %>% pivot_longer(cols=qw(pct_cum_env,pct_cum_evo,pct_cum_mut,pct_cum_rwr),names_to = qw(Source),values_to =qw(Fraction_pct))
my_df[my_df$Source=="pct_cum_env",]$Source<-"Env"
my_df[my_df$Source=="pct_cum_evo",]$Source<-"Evo"
my_df[my_df$Source=="pct_cum_mut",]$Source<-"Mut"
my_df[my_df$Source=="pct_cum_rwr",]$Source<-"Rwr"
my_df$Source<-factor(my_df$Source,levels=qw(Env,Evo,Mut,Rwr))

ggline(my_df,x="PC",y="Fraction_pct",numeric.x.axis = T,color="Source",
       # palette = c("#619CFF","#F8766D","#00BA38","#925E9FFF"),
       palette = c(mydscolpal,"#925E9FFF"),
       legend.title="",plot_type ="l",legend="right",
       xlab="Components of Env",ylab="Variance explained (%)")+
  scale_x_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,80))+
  scale_y_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,100))
#TODO ggsave(file="FigS13e.svg", path=my_supplfigout,dpi = 300, width = 4, height = 3)

Sys.time()
```
### 11.4.4. Perform PCA for each dataset independently
```{r Rwr,warning=FALSE, message=FALSE,fig.height=4.5,fig.width=11,dpi=300}
pca_res_env<-prcomp(dat_scaled_env,scale. = T)
pca_res_evo<-prcomp(dat_scaled_evo,scale. = T)
pca_res_mut<-prcomp(dat_scaled_mut,scale. = T)
pca_res_rwr<-prcomp(dat_scaled_rwr,scale. = T)

mypcs<-paste("PC",seq(1:3),sep="")

# Env vs. Rwr
temp5<-data.frame(Env_PC=rep(seq(1,3),each=3),Rwr_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp5)){
  temp1<-as.data.frame(pca_res_env$rotation[,temp5$Env_PC[i]])
  colnames(temp1)<-"rot_env"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(pca_res_rwr$rotation[,temp5$Rwr_PC[i]])
  colnames(temp2)<-"rot_rwr"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp5$Cor[i]<-cor.test(temp4$rot_env,temp4$rot_rwr, method = "spearman")$estimate
  temp5$p.value[i]<-cor.test(temp4$rot_env,temp4$rot_rwr, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp5.1)){
  for(j in 1:ncol(temp5.1)){
    temp5.1[i,j]<-temp5[temp5$Env_PC==i&temp5$Rwr_PC==j,]$Cor
    temp5.2[i,j]<-temp5[temp5$Env_PC==i&temp5$Rwr_PC==j,]$p.value
  }
}
temp5.3<-abs(temp5.1)

# Evo vs. Rwr
temp6<-data.frame(Evo_PC=rep(seq(1,3),each=3),Rwr_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp6)){
  temp1<-as.data.frame(pca_res_evo$rotation[,temp6$Evo_PC[i]])
  colnames(temp1)<-"rot_evo"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(pca_res_rwr$rotation[,temp6$Rwr_PC[i]])
  colnames(temp2)<-"rot_rwr"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp6$Cor[i]<-cor.test(temp4$rot_evo,temp4$rot_rwr, method = "spearman")$estimate
  temp6$p.value[i]<-cor.test(temp4$rot_evo,temp4$rot_rwr, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp6.1)){
  for(j in 1:ncol(temp6.1)){
    temp6.1[i,j]<-temp6[temp6$Evo_PC==i&temp6$Rwr_PC==j,]$Cor
    temp6.2[i,j]<-temp6[temp6$Evo_PC==i&temp6$Rwr_PC==j,]$p.value
  }
}
temp6.3<-abs(temp6.1)
...
# Mut vs. Rwr
temp7<-data.frame(Mut_PC=rep(seq(1,3),each=3),Rwr_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp7)){
  temp1<-as.data.frame(pca_res_mut$rotation[,temp7$Mut_PC[i]])
  colnames(temp1)<-"rot_mut"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(pca_res_rwr$rotation[,temp7$Rwr_PC[i]])
  colnames(temp2)<-"rot_rwr"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp7$Cor[i]<-cor.test(temp4$rot_mut,temp4$rot_rwr, method = "spearman")$estimate
  temp7$p.value[i]<-cor.test(temp4$rot_mut,temp4$rot_rwr, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp7.1)){
  for(j in 1:ncol(temp7.1)){
    temp7.1[i,j]<-temp7[temp7$Mut_PC==i&temp7$Rwr_PC==j,]$Cor
    temp7.2[i,j]<-temp7[temp7$Mut_PC==i&temp7$Rwr_PC==j,]$p.value
  }
}
temp7.3<-abs(temp7.1)
...
p<-grid.arrange(p4,p5,p6,as_ggplot(leg2),widths=c(1,1,1,0.4),nrow=1)
#TODO ggsave(file="FigS13d.svg",p, path=my_supplfigout,dpi = 300, width = 11, height = 4.5)

Sys.time()
```
### 11.4.5. PCA for averaged Rwr data
```{r Rwr,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
temp0<-Bstkdat_av
temp0<-temp0[order(temp0$Bnb,decreasing = F),]
rownames(temp0)<-temp0$Bnb
temp0<-na.omit(temp0[,which(colnames(temp0) %in% Bstk_sampleinfo$Prom_ORF)])
temp0<-t(temp0) # transpose the matrix so genes are as columns
figtemp<-as.data.frame(temp0)
figtemp<-figtemp %>% select_if(negate(anyNA))
res.pca.global_rw_av <- prcomp(figtemp, scale. = T)
# The PC scores are stored in the "x" value of the prcomp object
pc_scores_rw_av <- res.pca.global_rw_av$x %>% 
  # convert to a tibble retaining the sample names as a new column
  as_tibble(rownames = "Condition")
pc_scores_rw_av$label<-rownames(pc_scores_rw_av)

pc_eigenvalues_rw_av <- res.pca.global_rw_av$sdev^2
pc_eigenvalues_rw_av <- tibble(PC = factor(1:length(pc_eigenvalues_rw_av)), 
                         variance = pc_eigenvalues_rw_av) %>%   
  mutate(pct = variance/sum(variance)*100) %>% # add a new column with the percent variance
  mutate(pct_cum = cumsum(pct)) # add another column with the cumulative variance explained

# Scree plot
figtemp1<-pc_eigenvalues_rw_av
figtemp1$PC<-as.numeric(figtemp1$PC)
dscree_rw_av<-figtemp1
ggline(figtemp1,x="PC",y="pct_cum",numeric.x.axis = T,plot_type = "l",
        color="gray20",
       xlab="Components",ylab="Fraction of variance explained (%)")+
  # scale_x_continuous(breaks = get_breaks(by = 4, from = 0),limits = c(0,16))+
...
# Variance explatind by PC1~PCk
k<-3
print(paste("PC1~PC",k, " explains ", pc_eigenvalues_rw_av$pct_cum[k] ,"% variance",sep=""))


Sys.time()
```
### 11.4.6. 2D plot (PC1~PC3) for averaged data
```{r Rwr,warning=FALSE, message=FALSE,fig.height=2.5,fig.width=7.5,dpi=300}
figtemp<-pc_scores_rw_av
p1<-ggscatter(data=figtemp,x="PC1",y="PC2",
              alpha=0.5,
              legend="",
              color = "Condition",
  ...
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p1<-ggpar(p1,
      xlab = paste("PC1 (", round(pc_eigenvalues_rw_av$pct[1],digits = 0),"%)",sep=""),
      ylab = paste("PC2 (", round(pc_eigenvalues_rw_av$pct[2],digits = 0),"%)",sep=""))
...
p2<-ggscatter(data=figtemp,x="PC2",y="PC3",
  ...
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p2<-ggpar(p2,
      xlab = paste("PC2 (", round(pc_eigenvalues_rw_av$pct[2],digits = 0),"%)",sep=""),
      ylab = paste("PC3 (", round(pc_eigenvalues_rw_av$pct[3],digits = 0),"%)",sep=""))
...
p<-grid.arrange(p1,p2,p3,widths=c(1,1,1),nrow=1)
#TODO ggsave(file="FigS13c.svg",p, path=my_supplfigout,dpi = 300, width = 7.5, height = 2.5)
Sys.time()
```
### 11.4.7. 2D plot (PC1~PC3) for averaged data
```{r Rwr,warning=FALSE, message=FALSE,fig.height=4,fig.width=4,dpi=300}
figtemp<-pc_scores_rw_av
p3<-ggscatter(data=figtemp,x="PC1",y="PC3",
                 alpha=0.5,
                 legend="",
...
as_ggplot(leg)
#TODO ggsave(file="FigS13c_legend.svg",path=my_supplfigout,dpi = 300, width = 4, height = 4)

Sys.time()
```
### 11.4.8. Correlation between rotation (eigenvector) for PC1~PC3 for averaged data
```{r Rwr,warning=FALSE, message=FALSE,fig.height=4.5,fig.width=11,dpi=300}
my_lim<-c(0,0.65)
# Env vs. Rw
temp5<-data.frame(Env_PC=rep(seq(1,3),each=3),Rw_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp5)){
  ...
  colnames(temp1)<-"rot.env"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(res.pca.global_rw_av$rotation[,temp5$Rw_PC[i]])
  colnames(temp2)<-"rot.rw"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp5$Cor[i]<-cor.test(temp4$rot.env,temp4$rot.rw, method = "spearman")$estimate
...
for(i in 1:nrow(temp6)){
  ...
  colnames(temp1)<-"rot.evo"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(res.pca.global_rw_av$rotation[,temp6$Rw_PC[i]])
  colnames(temp2)<-"rot.rw"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp6$Cor[i]<-cor.test(temp4$rot.evo,temp4$rot.rw, method = "spearman")$estimate
...
for(i in 1:nrow(temp7)){
  ...
  colnames(temp1)<-"rot.mut"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(res.pca.global_rw_av$rotation[,temp7$Rw_PC[i]])
  colnames(temp2)<-"rot.rw"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp7$Cor[i]<-cor.test(temp4$rot.mut,temp4$rot.rw, method = "spearman")$estimate
...
Sys.time()
```
# 12. The Random datasets (DS1~3)
## 12.1. Many genes
2622 genes
### 12.1.1. perform PCA
```{r Random,fig.height=4.5,fig.width=11}
my_gene<-dBnb.comp %>% nrow()
my_ncond<-76
my_mean<-0
my_sd<-1

# generate dataset 1
dat_rnd<-createEmptyDf(nrow=my_gene,ncol=my_ncond,colnames=paste("cond",seq(1,my_ncond),sep="_"))
for(i in 1:nrow(dat_rnd)){
  dat_rnd[i,]<-rnorm(n = ncol(dat_rnd),mean = my_mean,sd = my_sd)
}
my_dat_ds1<-dat_rnd
my_dat_ds1$gene<-paste("gene",seq(1,nrow(my_dat_ds1)),sep="_")
my_dat_ds1<-my_dat_ds1 %>% select(gene,everything())
my_dat2<-my_dat_ds1 %>% select(-gene)
rownames(my_dat2)<-my_dat_ds1$gene
my_dat2<-my_dat2 %>% t() %>% as.data.frame() # transpose the matrix so genes are as columns
pca_res_ds1 <- prcomp(my_dat2, scale. = T) # perform PCA
...
my_dat_ds2<-dat_rnd
my_dat_ds2$gene<-paste("gene",seq(1,nrow(my_dat_ds2)),sep="_")
my_dat_ds2<-my_dat_ds2 %>% select(gene,everything())
my_dat2<-my_dat_ds2 %>% select(-gene)
rownames(my_dat2)<-my_dat_ds2$gene
my_dat2<-my_dat2 %>% t() %>% as.data.frame() # transpose the matrix so genes are as columns
pca_res_ds2 <- prcomp(my_dat2, scale. = T) # perform PCA
...
my_dat_ds3<-dat_rnd
my_dat_ds3$gene<-paste("gene",seq(1,nrow(my_dat_ds3)),sep="_")
my_dat_ds3<-my_dat_ds3 %>% select(gene,everything())
my_dat2<-my_dat_ds3 %>% select(-gene)
rownames(my_dat2)<-my_dat_ds3$gene
my_dat2<-my_dat2 %>% t() %>% as.data.frame() # transpose the matrix so genes are as columns
pca_res_ds3 <- prcomp(my_dat2, scale. = T) # perform PCA
...
mypcs<-paste("PC",seq(1:3),sep="")
# DS1 vs. DS2
temp5<-data.frame(DS1_PC=rep(seq(1,3),each=3),DS2_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp5)){
  temp1<-as.data.frame(pca_res_ds1$rotation[,temp5$DS1_PC[i]])
  colnames(temp1)<-"rot.ds1"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(pca_res_ds2$rotation[,temp5$DS2_PC[i]])
  colnames(temp2)<-"rot.ds2"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp5$Cor[i]<-cor.test(temp4$rot.ds1,temp4$rot.ds2, method = "spearman")$estimate
  temp5$p.value[i]<-cor.test(temp4$rot.ds1,temp4$rot.ds2, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp5.1)){
  for(j in 1:ncol(temp5.1)){
    temp5.1[i,j]<-temp5[temp5$DS1_PC==i&temp5$DS2_PC==j,]$Cor
    temp5.2[i,j]<-temp5[temp5$DS1_PC==i&temp5$DS2_PC==j,]$p.value
  }
}
temp5.3<-abs(temp5.1)

# DS1 vs. DS3
temp6<-data.frame(DS1_PC=rep(seq(1,3),each=3),DS3_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp6)){
  temp1<-as.data.frame(pca_res_ds1$rotation[,temp6$DS1_PC[i]])
  colnames(temp1)<-"rot.ds1"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(pca_res_ds3$rotation[,temp6$DS3_PC[i]])
  colnames(temp2)<-"rot.ds3"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp6$Cor[i]<-cor.test(temp4$rot.ds1,temp4$rot.ds3, method = "spearman")$estimate
  temp6$p.value[i]<-cor.test(temp4$rot.ds1,temp4$rot.ds3, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp6.1)){
  for(j in 1:ncol(temp6.1)){
    temp6.1[i,j]<-temp6[temp6$DS1_PC==i&temp6$DS3_PC==j,]$Cor
    temp6.2[i,j]<-temp6[temp6$DS1_PC==i&temp6$DS3_PC==j,]$p.value
  }
}
temp6.3<-abs(temp6.1)

# DS2 vs. DS3
temp7<-data.frame(DS3_PC=rep(seq(1,3),each=3),DS2_PC=rep(seq(1,3),3),Cor=NA,p.value=NA)
for(i in 1:nrow(temp7)){
  temp1<-as.data.frame(pca_res_ds3$rotation[,temp7$DS3_PC[i]])
  colnames(temp1)<-"rot.ds3"
  temp1$Bnb<-rownames(temp1)
  temp2<-as.data.frame(pca_res_ds2$rotation[,temp7$DS2_PC[i]])
  colnames(temp2)<-"rot.ds2"
  temp2$Bnb<-rownames(temp2)
  temp4<-left_join(temp1,temp2)
  temp7$Cor[i]<-cor.test(temp4$rot.ds3,temp4$rot.ds2, method = "spearman")$estimate
  temp7$p.value[i]<-cor.test(temp4$rot.ds3,temp4$rot.ds2, method = "spearman")$p.value
}
...
for(i in 1:nrow(temp7.1)){
  for(j in 1:ncol(temp7.1)){
    temp7.1[i,j]<-temp7[temp7$DS3_PC==i&temp7$DS2_PC==j,]$Cor
    temp7.2[i,j]<-temp7[temp7$DS3_PC==i&temp7$DS2_PC==j,]$p.value
  }
}
temp7.3<-abs(temp7.1)
...
p<-grid.arrange(p4,p5,p6,as_ggplot(leg2),widths=c(1,1,1,0.4),nrow=1)
#TODO ggsave(file="FigS11c.svg",p, path=my_supplfigout,dpi = 300, width = 11, height = 4.5)


Sys.time()
```
### 12.1.2. PC scores
```{r Random,fig.height=2.5,fig.width=7.5}
# The PC scores
## stored in the "x" value of the prcomp object
pc_scores_ds1 <-pca_res_ds1$x %>%
  as_tibble(rownames="Condition")
pc_scores_ds2 <-pca_res_ds2$x %>%
  as_tibble(rownames="Condition")
pc_scores_ds3 <-pca_res_ds3$x %>%
  as_tibble(rownames="Condition")

# Eigenvalues
pc_eigenvalues_ds1 <- pca_res_ds1$sdev^2
pc_eigenvalues_ds1 <- tibble(PC = factor(1:length(pc_eigenvalues_ds1)), 
                         variance = pc_eigenvalues_ds1) %>%   
  mutate(pct = variance/sum(variance)*100) %>% # add a new column with the percent variance
  mutate(pct_cum = cumsum(pct)) # add another column with the cumulative variance explained
pc_eigenvalues_ds2 <- pca_res_ds2$sdev^2
pc_eigenvalues_ds2 <- tibble(PC = factor(1:length(pc_eigenvalues_ds2)), 
                         variance = pc_eigenvalues_ds2) %>%   
  mutate(pct = variance/sum(variance)*100) %>% # add a new column with the percent variance
  mutate(pct_cum = cumsum(pct)) # add another column with the cumulative variance explained
pc_eigenvalues_ds3 <- pca_res_ds3$sdev^2
pc_eigenvalues_ds3 <- tibble(PC = factor(1:length(pc_eigenvalues_ds3)), 
                         variance = pc_eigenvalues_ds3) %>%   
  mutate(pct = variance/sum(variance)*100) %>% # add a new column with the percent variance
  mutate(pct_cum = cumsum(pct)) # add another column with the cumulative variance explained


# PC1 vs. PC2 for ds1
p1<-ggscatter(data=pc_scores_ds1,x="PC1",y="PC2",alpha=0.5,
                 color = "Condition",
                 legend="")+
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p1<-ggpar(p1,
      xlab = paste("PC1 (", round(pc_eigenvalues_ds1$pct[1],digits = 0),"%)",sep=""),
      ylab = paste("PC2 (", round(pc_eigenvalues_ds1$pct[2],digits = 0),"%)",sep=""))

# PC vs. PC2 for ds2
p2<-ggscatter(data=pc_scores_ds2,x="PC1",y="PC2",alpha=0.5,
                 color = "Condition",
                 legend="")+
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p2<-ggpar(p2,
      xlab = paste("PC1 (", round(pc_eigenvalues_ds2$pct[1],digits = 0),"%)",sep=""),
      ylab = paste("PC2 (", round(pc_eigenvalues_ds2$pct[2],digits = 0),"%)",sep=""))

# PC vs. PC2 for ds2
p3<-ggscatter(data=pc_scores_ds3,x="PC1",y="PC2",alpha=0.5,
                 color = "Condition",
                 legend="")+
  scale_y_continuous(expand = expansion(mult = c(0.2, 0.1)))
p3<-ggpar(p3,
      xlab = paste("PC1 (", round(pc_eigenvalues_ds3$pct[1],digits = 0),"%)",sep=""),
      ylab = paste("PC2 (", round(pc_eigenvalues_ds3$pct[2],digits = 0),"%)",sep=""))

p<-p1+p2+p3
p
#TODO ggsave(file="FigS11b.svg",p, path=my_supplfigout,dpi = 300, width = 7.5, height = 2.5)

# Scatter plot
my_df<-my_dat_ds1[c(1,2),] %>% t()
my_df<-data.frame(cond=rownames(my_df)[2:nrow(my_df)],
                  gene_1=my_df[2:nrow(my_df),1],
                  gene_2=my_df[2:nrow(my_df),2])
my_df$gene_1<-my_df$gene_1 %>% as.numeric()
my_df$gene_2<-my_df$gene_2 %>% as.numeric()
p4<-ggscatter(my_df,x="gene_1",y="gene_2",
              cor.coef = T,cor.method = "spearman",
              color="cond",legend="",alpha=0.5)+
  scale_y_continuous(breaks = get_breaks(by = 2,from = -4),limits = c(-3,5))

my_df<-my_dat_ds2[c(1,2),] %>% t()
my_df<-data.frame(cond=rownames(my_df)[2:nrow(my_df)],
                  gene_1=my_df[2:nrow(my_df),1],
                  gene_2=my_df[2:nrow(my_df),2])
my_df$gene_1<-my_df$gene_1 %>% as.numeric()
my_df$gene_2<-my_df$gene_2 %>% as.numeric()
p5<-ggscatter(my_df,x="gene_1",y="gene_2",
              cor.coef = T,cor.method = "spearman",
              color="cond",legend="",alpha=0.5)+
  scale_y_continuous(breaks = get_breaks(by = 2,from = -4),limits = c(-3,5))

my_df<-my_dat_ds3[c(1,2),] %>% t()
my_df<-data.frame(cond=rownames(my_df)[2:nrow(my_df)],
                  gene_1=my_df[2:nrow(my_df),1],
                  gene_2=my_df[2:nrow(my_df),2])
my_df$gene_1<-my_df$gene_1 %>% as.numeric()
my_df$gene_2<-my_df$gene_2 %>% as.numeric()
p6<-ggscatter(my_df,x="gene_1",y="gene_2",
              cor.coef = T,cor.method = "spearman",
              color="cond",legend="",alpha=0.5)+
  scale_y_continuous(breaks = get_breaks(by = 2,from = -4),limits = c(-3,5))
...
p<-p4+p5+p6
p
#TODO ggsave(file="FigS11a.svg",p, path=my_supplfigout,dpi = 300, width = 7.5, height = 2.5)

Sys.time()
```
### 12.1.3. Mapping DS2, and 3 onto the PC space defined by DS1
```{r Random,fig.height=3,fig.width=3}
# scaling of ds2
temp0<-my_dat_ds2
rownames(temp0)<-temp0$gene
temp0<-temp0 %>% select(-gene)
dscale<-data.frame(gene=rownames(temp0),center=rowMeans(temp0),scale=rowSds(as.matrix(temp0)))
temp0<-t(temp0) # transpose the matrix so genes are as columns
temp<-scale(temp0) # apply scaling to each column of the matrix (genes)

print(sum(pc_eigenvalues_ds1$variance)) # this should be the number of genes in the dataset (i.e. identical to nrow(dBnb.com))
# Calc % of variance of the Env explaind py PCs 
figtemp1<-pc_eigenvalues_ds1
figtemp1$PC<-as.numeric(figtemp1$PC)
figtemp1$variance.ds2<-NA
figtemp1$pct.ds2<-NA
figtemp1$pct_cum.ds2<-NA
i<-1
for(i in 1:nrow(figtemp1)){
  temp2<-temp %*% pca_res_ds1$rotation[,i]
  figtemp1$variance.ds2[i]<-var(temp2)
}
figtemp1$pct.ds2<-figtemp1$variance.ds2/sum(pc_eigenvalues_ds1$variance)*100
figtemp1$pct_cum.ds2<-cumsum(figtemp1$pct.ds2)
...
temp<-scale(temp0) # apply scaling to each column of the matrix (genes)
# Calc % of variance of the Env explaind py PCs 
figtemp1$variance.ds3<-NA
figtemp1$pct.ds3<-NA
figtemp1$pct_cum.ds3<-NA
i<-1
for(i in 1:nrow(figtemp1)){
  temp2<-temp %*% pca_res_ds1$rotation[,i]
  figtemp1$variance.ds3[i]<-var(temp2)
}
figtemp1$pct.ds3<-figtemp1$variance.ds3/sum(pc_eigenvalues_ds1$variance)*100
figtemp1$pct_cum.ds3<-cumsum(figtemp1$pct.ds3)
dscree_ds123<-figtemp1


figtemp2<-dscree_ds123 %>% pivot_longer(cols=qw(pct_cum,pct_cum.ds2,pct_cum.ds3),names_to = qw(Source),values_to =qw(Fraction_pct))
figtemp2[figtemp2$Source=="pct_cum",]$Source<-"DS1"
figtemp2[figtemp2$Source=="pct_cum.ds2",]$Source<-"DS2"
figtemp2[figtemp2$Source=="pct_cum.ds3",]$Source<-"DS3"
figtemp2$Source<-factor(figtemp2$Source,levels=qw(DS1,DS2,DS3))
ggline(figtemp2,x="PC",y="Fraction_pct",numeric.x.axis = T,color="Source",
       # palette = c("#619CFF","#F8766D","#00BA38"),
       palette = mydscolpal,
       legend.title="",plot_type ="l",linetype = "Source",
       xlab="Components of DS1",ylab="Variance explained (%)")+
  scale_x_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,80))+
  scale_y_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,100))
#TODO ggsave(file="FigS11d.svg", path=my_supplfigout,dpi = 300, width = 3, height = 3)

Sys.time()
```
## 12.2. Fewer genes
26 genes (1/100 of 2622)
### 12.2.1. perform PCA
```{r Random,fig.height=4.5,fig.width=11}
my_gene<-26
my_ncond<-76
...
dat_rnd<-createEmptyDf(nrow=my_gene,ncol=my_ncond,colnames=paste("cond",seq(1,my_ncond),sep="_"))
for(i in 1:nrow(dat_rnd)){
  dat_rnd[i,]<-rnorm(n = ncol(dat_rnd),mean = 1,sd = 1)
}
my_dat_ds1<-dat_rnd
my_dat_ds1$gene<-paste("gene",seq(1,nrow(my_dat_ds1)),sep="_")
my_dat_ds1<-my_dat_ds1 %>% select(gene,everything())
...
Sys.time()
```
### 12.2.3. Mapping DS2, and 3 onto the PC space defined by DS1
```{r Random,fig.height=3,fig.width=3}
# scaling of ds2
temp0<-my_dat_ds2
rownames(temp0)<-temp0$gene
...
figtemp2[figtemp2$Source=="pct_cum.ds3",]$Source<-"DS3"
figtemp2$Source<-factor(figtemp2$Source,levels=qw(DS1,DS2,DS3))
ggline(figtemp2,x="PC",y="Fraction_pct",numeric.x.axis = T,color="Source",palette = c("#619CFF","#F8766D","#00BA38"),
       legend.title="",plot_type ="l",linetype = "Source",
       xlab="Components",ylab="Fraction variance explained (%)")+
  scale_x_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,80))+
  scale_y_continuous(breaks = get_breaks(by = 20, from = 0),limits = c(0,100))
...
Sys.time()
```
# 13. Noise-Plasticity Coupling
## 13.1. The Noise_ch dataset
Taniguchi 2010, Science
https://doi.org/10.1126/science.1188308
Strain: YFP Fusion Library
### 13.1.1. Load the Noise_ch dataset
```{r Noise_ch,warning=FALSE, message=FALSE,fig.height=3,fig.width=3.5,dpi=300}
...
ggscatter(df_tani,x="TNMean",y="TNNoise",alpha=0.5,size=0.3,color = "HL_label",palette = c("green4","green3"),legend="",
          cor.coef = TRUE,cor.coeff.args = list(method = "spearman", label.x = 1.5, label.y=0.8, label.sep = "\n",size=4))+
            xlab(expression(log[10]~"protein expression level [arb. unit]"))+
            ylab(expression(log[10]~"protein noise [arb. unit]"))+
...
#TODO ggsave(file="FigS24c.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
ggscatter(df_tani,x="TNMean",y="TNDM",
  ...
  xlab(expression(log[10]~"protein expression level [arb. unit]"))+
  ylab(expression(DM[noise_ch]~"[arb. unit]"))
#TODO ggsave(file="FigS24d.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)

Sys.time()
```
### 13.1.2. Basic statistics of the dataset
...
p<-p1+p2+p3
p
#TODO ggsave(file="FigS24f.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)
...
p<-p4+p5+p6
p
#TODO ggsave(file="FigS24e.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)
...
### 14.3. DM
#### 14.3.1. Calc DM
```{r Prtn,warning=FALSE, message=FALSE,fig.height=3,fig.width=3.5,dpi=300}
...
ggscatter(df_prtn_sch,x="mean",y="sd",
          ...
          alpha = 0.2, size=0.3,legend="")+
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"protein expression level [arb. unit]"))+
  ylab(expression("Standard deviation [arb. unit]"))+
  geom_line(data=df_prtn_sch,aes(x=mean,y=RunmedVar), colour="darkorange", size=0.5)+
  geom_line(data=pred,aes(x=x,y=smoothsp), colour="deepskyblue", size=0.5) # "deepskyblue"
#TODO ggsave(file="FigS17c.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
ggscatter(df_prtn_sch,x="mean",y="DMprtn",
  ...
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"protein expression level [arb. unit]"))+
  ylab(expression(DM[prtn]~"[arb. unit]"))
#TODO ggsave(file="FigS17d.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
p<-p1+p2+p3
p
#TODO ggsave(file="FigS17e.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,1.2),nrow=1)
#TODO ggsave(file="FigS18a.svg",p, path=my_supplfigout,dpi = 300, width = 12, height = 3)
Sys.time()
```
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,1.2),nrow=1)
#TODO ggsave(file="FigS18b.svg",p, path=my_supplfigout,dpi = 300, width = 12, height = 3)
Sys.time()
```
...
## 15.3. DM 
### 15.3.1. Calc DM
```{r Strs,warning=FALSE, message=FALSE,fig.height=3,fig.width=3.5,dpi=300}
...
ggscatter(df_strs_maeiwa,x="mean",y="sd",
  ...
  geom_line(data=pred,aes(x=x,y=smoothsp), colour="deepskyblue", size=0.5) # "deepskyblue"
#TODO ggsave(file="FigS22d.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
ggscatter(df_strs_maeiwa,x="mean",y="DMstrs",
  ...
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
  ylab(expression(DM[strs]~"[arb. unit]"))
#TODO ggsave(file="FigS22e.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
p<-p1+p2+p3
p
#TODO ggsave(file="FigS22f.svg", path=my_supplfigout,dpi = 300, width = 9, height = 3)
...
p4<-ggscatter(df_strs_maeiwa,x="DMenv_wo_abx",y="DMstrs",
  ...
  scale_x_continuous(breaks = get_breaks(by = 1, from = 0),limits =  c(-0.7, 3.5))+
  scale_y_continuous(breaks = get_breaks(by = 1, from = 0),limits =  c(-0.2, 2.1))+
  xlab(expression(DM[env_wo_abx]~"[arb. unit]"))+
  ylab(expression(DM[strs]~"[arb. unit]"))
p4<-ggpar(p4,font.y=my_fontsize,font.x=my_fontsize,font.xtickslab = my_fontsize,font.ytickslab = my_fontsize)
p4+blank+blank
Sys.time()
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,1),nrow=1)
#TODO ggsave(file="FigS23.svg",p, path=my_supplfigout,dpi = 300, width = 12, height = 4)
Sys.time()
```
...
p9<-ggviolin(figtemp1,x="TGofCommonMutTR_flag",y="DMenv_wo_abx",fill="TGofCommonMutTR_flag",legend="",xlab="",add="boxplot",palette = c("#00AFBB", "#FC4E07"), add.params = list(fill = "white"))+
  ...
  ylab(expression(DM[env_wo_abx]~"[arb. unit]"))+
  annotate("text",x=1,y=Inf,label= nrow(figtemp1[(!is.na(figtemp1$DMenv))&(figtemp1$TGofCommonMutTR_flag=="Others"),]),hjust = 0.5,vjust = 4)+
  annotate("text",x=2,y=Inf,label= nrow(figtemp1[(!is.na(figtemp1$DMenv))&(figtemp1$TGofCommonMutTR_flag=="TGs of common mutTRs"),]),hjust = 0.5,vjust = 4)+
  scale_x_discrete(labels = c("Targets of\nother TRs","Targets of\ncommonly\nmuated TRs"))+
...
p1<-ggscatter(my_df4,x="Co",y="Expression",alpha=0.5,color = "Type",
  ...
  xlab(expression(atop(log[2]~"mRNA expression level","of control strains [arb. unit]")))+
  ylab(expression(atop(log[2]~"mRNA expression level","of resistant mutants [arb. unit]")))+
...
p2<-ggboxplot(my_df3,x="Type",y="log2_fold_change",color = "Type",size=1,
  ...
  ylab(expression(log[2]~"fold change [arb. unit]"))+
  scale_y_continuous(expand = expansion(mult = c(0.1, 0.1)))+
...
### 16.1.2. DM
#### 16.1.2.1. Calc DM
```{r Strs_hp,warning=FALSE, message=FALSE,fig.height=3,fig.width=3.5,dpi=300}
...
ggscatter(df_strs_hori,x="mean",y="sd",
  ...
  geom_line(data=pred,aes(x=x,y=smoothsp), colour="deepskyblue", size=0.5) # "deepskyblue"
#TODO ggsave(file="FigS19e_Strs_hp.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
ggscatter(df_strs_hori,x="mean",y="DMstrs",
  ...
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
  ylab(expression(DM[strs_hp]~"[arb. unit]"))
#TODO ggsave(file="FigS19f_Strs_hp.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
p<-p1+p2+p3
p
#TODO ggsave(file="FigS20a.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)

Sys.time()
```
### 16.1.3. PCA
#### 16.1.3.1. Detect outliers in PC1~PC3
```{r Strs_hp,warning=FALSE, message=FALSE,fig.height=4,fig.width=12,dpi=300}
temp0<-Horidat
temp0<-temp0[order(temp0$Bnb,decreasing = F),]
rownames(temp0)<-temp0$Bnb
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,0.3),nrow=1)
#TODO ggsave(file="FigS21a.svg",p, path=my_supplfigout,dpi = 300, width = 11, height = 3)
Sys.time()
```
...
### 16.2.2. DM
#### 16.2.2.1. Calc DM
```{r Strs_he,warning=FALSE, message=FALSE,fig.height=3,fig.width=3.5,dpi=300}
...
ggscatter(df_strs_hori_evo,x="mean",y="sd",
  ...
  geom_line(data=pred,aes(x=x,y=smoothsp), colour="deepskyblue", size=0.5) # "deepskyblue"
#TODO ggsave(file="FigS19e_Strs_he.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
ggscatter(df_strs_hori_evo,x="mean",y="DMstrs",
  ...
  # scale_x_continuous(breaks = get_breaks(by = 5, from = 0),limits =  c(0, 12))+
  xlab(expression(log[2]~"mRNA expression level [arb. unit]"))+
  ylab(expression(DM[strs_he]~"[arb. unit]"))
#TODO ggsave(file="FigS19f_Strs_he.svg", path=my_supplfigout,dpi = 300, width = 3.5, height = 3)
...
p<-p1+p2+p3
p
#TODO ggsave(file="FigS20b.svg",p, path=my_supplfigout,dpi = 300, width = 9, height = 3)

Sys.time()
```
### 16.2.3. Examine PCA
#### 16.2.3.1. Detect outliers in PC1~PC3
```{r Strs_he,warning=FALSE, message=FALSE,fig.height=4,fig.width=12,dpi=300}
temp0<-Hori_evo_dat
temp0<-temp0[order(temp0$Bnb,decreasing = F),]
rownames(temp0)<-temp0$Bnb
...
p<-grid.arrange(p1,p2,p3,as_ggplot(leg),widths=c(1,1,1,0.3),nrow=1)
#TODO ggsave(file="FigS21b.svg",p, path=my_supplfigout,dpi = 300, width = 11, height = 3)
Sys.time()
```
...
g_core %>% 
  ...
  # ggraph::geom_node_point(aes(x=x*1.05,y=y*1.05,fill=impact_flag), size=5,shape=21)+scale_fill_manual(values=c("grey50","grey50","#ED0000FF"))+
  ggraph::geom_node_text(aes(x = x*1.2, y=y*1.2,angle=angle, label=lab, hjust=hjust,colour=sensor_flag), size=7) +
  scale_color_manual(values = c("#925E9FFF","#1B1919EF"))+
  ggraph::theme_graph(background="white",)+
  theme(plot.margin=unit(c(0,0,0,0),"cm"))+
  expand_limits(x = c(-2, 2), y = c(-2, 2))+
  scale_size(range = c(5, 10))
#TODO ggsave(file="Fig5d.svg", path=my_figout,dpi = 300, width = 10, height = 9)
...
ggbarplot(df_bt_stat_long,x="label",y="size",fill="regulator_flag",palette = c("#ED0000FF","#ADB6B6FF"),
          legend.title="",legend="right",xlab="",ylab="Number of nodes",ylim=c(0,200),
  position = position_dodge(0.9),size=0)+rotate_x_text()
#TODO ggsave(file="Fig5e.svg", path=my_figout,dpi = 300, width = 3, height = 3)

ggbarplot(df_bt_stat_long,x="label",y="size_sr",fill="sensor_flag",
          legend.title="",legend="right",xlab="",ylab="Number of TR",palette = c("#925E9FFF", "#1B1919EF"),
  position = position_dodge(0.9),size=0)+rotate_x_text()
#TODO ggsave(file="Fig5f.svg", path=my_figout,dpi = 300, width = 3, height = 3)
...
temp<-igraph::as_data_frame(g_stml,what = "vertices")
...
as.ggplot(function() plot(g_stml, vertex.color=V(g_stml)$node_color, edge.arrow.size=0.5, layout=LO,vertex.size=V(g_stml)$node_size*3,vertex.label="",
     edge.color=E(g_stml)$edge_col,edge.lty=E(g_stml)$edge_lty))
#TODO ggsave(file="Fig5c.svg", path=my_figout,dpi = 300, width = 23, height = 20)
...
Self loops are omitted.
```{r Bowtie,warning=FALSE, message=FALSE,fig.height=3.1,fig.width=3,dpi=300}
temp0<-trn.stat.table[trn.stat.table$all_target>4,] %>% distinct(regulator,.keep_all = T)
...
```{r Bowtie,warning=FALSE, message=FALSE,fig.height=3.1,fig.width=6,dpi=300}
...
p
#TODO ggsave(file="Fig7b.svg",p, path=my_figout,dpi = 300, width = 4, height = 3)
...
p<-ggpar(p,legend.title = "|R|",font.legend = my_fontsize)+theme(axis.ticks.x = element_blank(),axis.ticks.y = element_blank())
p
#TODO ggsave(file="Fig7c_Env.svg",p, path=my_figout,dpi = 300, width = 4, height = 6)
...
p<-ggpar(p,legend.title = "|R|",font.legend = my_fontsize)+theme(axis.ticks.x = element_blank(),axis.ticks.y = element_blank())
p
#TODO ggsave(file="Fig7c_Evo.svg",p, path=my_figout,dpi = 300, width = 4, height = 6)
...
p<-ggpar(p,legend.title = "|R|",font.legend = my_fontsize)+theme(axis.ticks.x = element_blank(),axis.ticks.y = element_blank())
p
#TODO ggsave(file="Fig7c_Mut.svg",p, path=my_figout,dpi = 300, width = 4, height = 6)

Sys.time()
```
# 19. Output for Supplementary Data
## 19.1. Output tables
```{r Output,warning=FALSE, message=FALSE,fig.height=6,fig.width=9,dpi=300}
