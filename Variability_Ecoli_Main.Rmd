...
Section 1: Define name space of genes and set TRN, KEGG pathway
...
Section 5: Comparison of DMs
Section 6: GSEA in KEGG pathways
Section 7: GSEA in TRN
...
Section 17: Bow-tie deconvolution
Section 18:
Section 19:
...
# load("./MATranscriptome_submit.RData")
```
## 0.2. Save workspace
```{r workspace_save,warning=FALSE, message=FALSE}
# save.image("./MATranscriptome_submit.RData")
```
## 0.3. Library loading
...
```{r Mut,warning=FALSE, message=FALSE,fig.height=2.5,fig.width=3,dpi=300}
my_fontsize<-16
...
```{r Env,warning=FALSE, message=FALSE,fig.height=2.5,fig.width=3,dpi=300}
my_fontsize<-16
# Mean vs SD in PR2
...
# 5. Comparison of DM values between all datasets
...
# ggplot(dot_df, aes(x = enrichmentScore, y = fct_reorder(Pathway_name, enrichmentScore))) + 
#                geom_point(aes(size = GeneRatio, color = qvalues)) +
#                theme_bw(base_size = 14) +
#         scale_colour_gradient(limits=c(0, 0.05), low="red") +
#         # scale_y_discrete(labels = function(y) str_wrap(y, width = 30))+
#         ylab(NULL) +
#         ggtitle("Kegg pathway enrichment",subtitle =mysubtitle)
...
# p<-ggdotplot(dot_df[dot_df$qvalue<0.05,],y="enrichmentScore",x="Pathway_name",rotate=T, legend="",
#           fill = "qvalue",size=2,
#           sort.val = "desc",
#           ylab="Enrichment Score (GSEA)",xlab="")
# p
# q<-ggpar(p,legend = "right",legend.title = "q-value",font.ytickslab= c(10))
# leg <- ggpubr::get_legend(q)
# as_ggplot(leg)

figtemp1<-dot_df[dot_df$qvalue<0.05,]
...
Sys.time()
```
## 7.4. Venn diagram of enriched TFs in higher DM values in three datasets and define TRcom
...
# global_eTFs<-Evo[(Evo %in% Env)&(Evo %in% Mut)] #TRcoms
...
Sys.time()
```
## 7.5. Venn diagram of enriched TFs in lower DM values in three datasets
```{r All_GSEA_TRN,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# venn
venn_all<-list(
  Evo=dgsea_trn_evo[(dgsea_trn_evo$NES<0)&(dgsea_trn_evo$qvalue<0.05),]$ID,
...
Sys.time()
```
## 8.3. Subnetwork comprising TRcom
https://qiita.com/saltcooky/items/4e55d97c5e86dfb208cd 
...
my_labs<-c(bquote(TR[com]),bquote(Others),bquote(All),bquote(Random))
...
```{r TRN,warning=FALSE, message=FALSE,fig.height=3,fig.width=9,dpi=300}
lay<-rbind(seq(1,3))
grid.arrange(figlist[[1]],figlist[[2]],figlist[[3]],
             layout_matrix=lay,
             bottom=textGrob("Number of unique transcriptional regulators",gp=gpar(fontsize=16)),
             widths=c(1,1,1))
Sys.time()
```

## 8.12. Example of correlation between DMenv and the Number of TRcoms for target genes which has 5 unique regulators
### 8.12.1.1. Include all sigma factors
...
## 8.13.
Consider not only direct regulators but also indirect regulators
...
## 8.14.
```{r TRN,warning=FALSE, message=FALSE,fig.height=3,fig.width=6,dpi=300}
...
#    hjust = as.vector(matrix(c(0, 0, 0, 1, 1), ncol = 5, nrow = nrow(my_df), byrow = TRUE)),
#    x = as.vector(matrix(c(.1, .1, .01,.9, .9), ncol = 5, nrow = nrow(my_df), byrow = TRUE))
#  )
# )
# 
# ggtexttable(head(my_df,50),theme = table_theme)

my_df$label<-"Others"
my_df[my_df$gene=="selA",]$label<-"selA"
my_df[my_df$gene=="modA",]$label<-"modA"
my_df$label<-factor(my_df$label,levels=qw(modA,selA,Others))

my_df<-my_df[order(my_df$DMenv,decreasing = T),]
p1<-ggbarplot(my_df,x="gene",y="DMenv",size=0,fill="label",palette = c(mycolpal[c(2,1)],"gray50"),legend="")+
  rotate_x_text(angle=90)+ylab(expression(DM[env]))
p1<-ggpar(p1,font.x = c(30),font.y=c(30),font.ytickslab = c(30),font.xtickslab = c(3))
p1

my_df<-my_df[order(my_df$DMevo,decreasing = T),]
p2<-ggbarplot(my_df,x="gene",y="DMevo",size=0,fill="label",palette = c(mycolpal[c(2,1)],"gray50"),legend="")+
  rotate_x_text(angle=90)+ylab(expression(DM[evo]))
p2<-ggpar(p2,font.x = c(30),font.y=c(30),font.ytickslab = c(30),font.xtickslab = c(3))
p2

my_df<-my_df[order(my_df$DMmut,decreasing = T),]
p3<-ggbarplot(my_df,x="gene",y="DMmut",size=0,fill="label",palette = c(mycolpal[c(2,1)],"gray50"),legend="")+
  rotate_x_text(angle=90)+ylab(expression(DM[mut]))
p3<-ggpar(p3,font.x = c(30),font.y=c(30),font.ytickslab = c(30),font.xtickslab = c(3))
p3

Sys.time()
```
## 8.15. A toy data only for visualizasation of principle of GSEA
```{r TRN,warning=FALSE, message=FALSE,fig.height=3,fig.width=6,dpi=300}
temp<-dBnb.comp %>% select(Bnb,gene)
my_df<-left_join(df.regcount,temp,by=c("gene_id"="Bnb"))
my_df<-my_df %>% select(gene,DMenv,DMevo,DMmut,regulators)
...
p1<-ggbarplot(my_df,x="gene",y="DMenv",size=0.5,fill="label",color="label",palette = c("black","gray80"),legend="")+ # #E46726
  rotate_x_text(angle=90)+ylab(expression(DM[env]))
p1<-ggpar(p1,font.y=c(30),font.ytickslab = c(30))
...
Sys.time()
```
### 9.1.4. Mapping the Evo dataset onto the PC space defined by the Env dataset
...
Sys.time()
```
### 9.1.5. Mapping the Mut dataset (Anc, R10, and R30) onto the PC plane defined by ENV_DataSet
```{r PCA_Env,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
...
Sys.time()
```
### 9.1.6. Mapping MUT dataset (R30, R10, separately) onto the PC plane defined by ENV_DataSet
```{r PCA_Env,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# R30
...
Sys.time()
```
### 9.1.7. Scree plot for Env, Evo and  Mut
...
Sys.time()
```
## 9.6. GSEA for loadings of PCA using TRcom
### 9.6.1. GSEA TRcom for PCA loading (GSEA was performed using only TRcoms and distinguish differernt mode)
...
for (j in 1:length(my_datasets)){
  ...
  for(ii in 1:length(mypcs)){
    ...
    p<-ggdotchart(figtemp2[figtemp2$PC==k,],x="Description",y="NES",
      ...
      annotate("text",x=Inf,y=Inf,label=paste(mypcs[ii],"(",my_datasets[j],")"),hjust = 1,vjust = 1,size=5)+
...
Sys.time()
```
### 9.6.2. Venn diagram of enriched regulation of TRcom in PC1~PC3
PC1~PC3
...
Sys.time()
```
### 9.6.3. Venn diagram of enriched regulation of TRcom in PC1~PC3
PC1 only
```{r GSEA_TRcom,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# venn
venn_all<-list(
...
Sys.time()
```
## 9.7. GSEA for loadings of PCA using all TR
### 9.7.1. GSEA TR all for PCA loading (GSEA was performed using all TR (including TRcoms and other TRs) and distinguish differernt mode)
...
for (j in 1:length(my_datasets)){
  ...
  for(ii in 1:length(mypcs)){
    ...
    p<-ggdotchart(figtemp2[figtemp2$PC==k,],x="Description",y="NES",
      ...
      scale_size_continuous(limits = c(0,1),breaks=c(0.2,0.4,0.6,0.8))+
      # annotate("text",x=Inf,y=Inf,label=paste(mypcs[ii],"(",my_datasets[j],")"),hjust = 1,vjust = 1,size=5)+
      annotate("text",x=Inf,y=Inf,label=paste(mypcs[ii],"(",my_datasets[j],")",sep=""),hjust = 1,vjust = 1,size=8)+
       # scale_size_area(max_size = 4,breaks=c(0.3,0.4,0.5,0.6,0.7,0.8)) +
      guides(size = guide_legend(title= "Gene ratio",label.theme = element_text(angle = 0,size=my_fontsize),title.position = "left"))+
      scale_colour_continuous(limits = c(-4.1,log10(0.05)), breaks = c(-4,-3,-2,-1),
                        guide=guide_colourbar(title="q-value (log)",title.position = "left",label.vjust = 0,label.hjust = 0,label.theme = element_text(angle = 0,size=my_fontsize),ticks.linewidth = 1,order=1))
...
Sys.time()
```
### 9.7.2. Venn diagram of enriched regulation of TRs in PC1~PC3
PC1~PC3
...
Sys.time()
```
### 9.7.3. Venn diagram of enriched regulation of TR in PC1~PC3
PC1 only
```{r GSEA_allTR,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# venn
venn_all<-list(
...
Sys.time()
```
### 9.7.4. Venn diagram of enriched regulation of TR in PC1~PC3
PC1 for Env and PC2 for Evo&Mut
```{r GSEA_allTR,warning=FALSE, message=FALSE,fig.height=3,fig.width=3,dpi=300}
# venn
venn_all<-list(
  Evo=dTRall_pca_gsea[(dTRall_pca_gsea$dataset=="Evo")&(dTRall_pca_gsea$PC=="PC1"),]$ID,
  Env=dTRall_pca_gsea[(dTRall_pca_gsea$dataset=="Env")&(dTRall_pca_gsea$PC=="PC2"),]$ID,
  Mut=dTRall_pca_gsea[(dTRall_pca_gsea$dataset=="Mut")&(dTRall_pca_gsea$PC=="PC2"),]$ID
)
# ggVennDiagram(figtemp)
venn <- process_data(Venn(venn_all))
venn$regionData$name
...
```
# 10. Gene Ontology
## 10.1. Create custom GO slim (a subset of GO, dfgoslim2, dfgoslim23)
...
### 10.4.1. GSEA in GO for the Env dataset
...
Sys.time()
```
### 10.4.2. GSEA in GO for the Evo dataset
```{r GO_slim,warning=FALSE, message=FALSE,fig.height=10,fig.width=5,dpi=300}
my_DM<-"DMevo"
gene_list <- df_DMall[,colnames(df_DMall)==my_DM] # Create a vector of the gene universe
names(gene_list)<-df_DMall$Bnb
...
Sys.time()
```
### 10.4.3. GSEA in GO for the Mut dataset
```{r GO_slim,warning=FALSE, message=FALSE,fig.height=10,fig.width=5,dpi=300}
my_DM<-"DMmut"
gene_list <- df_DMall[,colnames(df_DMall)==my_DM] # Create a vector of the gene universe
names(gene_list)<-df_DMall$Bnb
...
### 10.5.1. GSEA in GO for the Env dataset
...
Sys.time()
```
### 10.5.2. GSEA in GO for the Evo dataset
```{r GO_slim,warning=FALSE, message=FALSE,fig.height=10,fig.width=10,dpi=300}
my_t2g<-dfgoslim3 %>% select(go,Bnb)
my_fontsize<-20
my_datasets<-qw(Evo)
...
Sys.time()
```
### 10.5.3. GSEA in GO for the Mut dataset
```{r GO_slim,warning=FALSE, message=FALSE,fig.height=10,fig.width=10,dpi=300}
my_t2g<-dfgoslim3 %>% select(go,Bnb)
my_fontsize<-20
my_datasets<-qw(Mut)
